...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/little-agent/Little-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    little_meta = (fm.get("metadata") or {}).get("little") or {}
    tags = little_meta.get("tags") or []
    related = little_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `little skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Little Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Little ships with a large built-in skill library copied into `~/.little/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Little also syncs bundled skills on `little update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.little/skills/` tree, it is still shipped with Little; restore it with `little skills reset <name> --restore`.",
        "",
