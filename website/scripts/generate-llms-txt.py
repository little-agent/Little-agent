#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Little docs site.
...
Both publish at:
  https://little-agent.little-agent.com/docs/llms.txt
  https://little-agent.little-agent.com/docs/llms-full.txt
...
SITE_BASE = "https://little-agent.little-agent.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Little as a Python Library", None),
        ("guides/use-mcp-with-little", "Use MCP with Little", None),
        ("guides/use-voice-mode-with-little", "Use Voice Mode with Little", None),
        ("guides/use-soul-with-little", "Use SOUL.md with Little", None),
        ("guides/build-a-little-plugin", "Build a Little Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Little"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Little Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Little Agent Team. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/little-agent/"
        "little-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/little-agent/Little-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Little Agent — Full Documentation\n",
        (
            "This file is the entire Little Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Little, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://little-agent.little-agent.com/docs\n",
        "Short index: https://little-agent.little-agent.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
