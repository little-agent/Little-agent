...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `little send`."
---

# Pipe Script Output to Messaging Platforms

`little send` is a small, scriptable CLI that pushes a message to any
messaging platform Little is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | little send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `little
...
little send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
little send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
little send --list

# Filter by platform
little send --list telegram
```
...
Any platform Little ships adapters for works as a target:
...
`little send` resolves the message body in this order:

1. **Positional argument** — `little send --to telegram "hi"`
2. **`--file PATH`** — `little send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | little send --to telegram`

When stdin is a TTY (no pipe), Little does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  little send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `little send` reuses your Little config, the same script works on
any host where Little is installed — no need to export bot tokens into
...
`little send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  little send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | little send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.little/bin/little send \
...
```bash
./train.py --epochs 200 && \
  little send --to telegram "training done" || \
  little send --to telegram "training failed (exit $?)"
```
...
little send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(little send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `little send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `little send` calls
the platform's REST endpoint directly using credentials from
`~/.little/.env` and `~/.little/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `little gateway start` and retry.
...
# Machine-readable
little send --list --json
```

The listing is built from `~/.little/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`little gateway
...
| Approach | Multi-platform | Reuses Little creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `little send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`little send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `little send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `little send` shares with cron delivery.
