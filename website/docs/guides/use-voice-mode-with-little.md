---
sidebar_position: 8
title: "Use Voice Mode with Little"
description: "A practical guide to setting up and using Little voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Little
...
- you want Little sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Little.
...
## Step 1: make sure normal Little works first
...
Little supports both local and cloud speech stacks.
...
### If you use `little setup`

If you choose NeuTTS in the setup wizard, Little checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Little transcribes and responds
...
- asking Little to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Little loop.

## Tuning CLI behavior

### Silence threshold

If Little starts/stops too aggressively, tune:
...
Little stays a normal chat bot, but can speak replies.
...
- you want Little to function like a portable research or ops assistant
...
Little joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Little detects speech boundaries
- transcripts are posted in the associated text channel
- Little responds in text and audio
...
1. get text Little working
2. install `little-agent[voice]`
