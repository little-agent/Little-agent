---
sidebar_position: 6
title: "Use MCP with Little"
description: "A practical guide to connecting MCP servers to Little Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Little

This guide shows how to actually use MCP with Little Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Little tool
- you want Little to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Little to internal APIs, databases, or company systems without modifying Little core

Do not use MCP when:
- a built-in Little tool already solves the job well
...
- Little remains the agent
- MCP servers contribute tools
- Little discovers those tools at startup or reload time
...
If you installed Little with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Little banner/status should show MCP integration when configured
- ask Little what tools it has available
...
## WSL2: bridge Little in WSL to Windows Chrome

This is the practical setup when:

- Little runs inside WSL2
...
In this setup, Little does **not** connect to Chrome directly. Instead:

- Little runs in WSL
- Little starts a local stdio MCP server
...
```text
Little (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Little stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Little core browser transport
...
```bash
little mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
little mcp test chrome-devtools-win
```

Then start a fresh Little session or run:
...
Once loaded, Little can use the MCP-prefixed browser tools directly. For example:
...
If Little runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Little from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Little from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Little:
...
2. Little-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Little will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Little to reason over a bounded workspace.
...
Now Little can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Little core.
...
Because Little now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
