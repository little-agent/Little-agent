...
This tutorial walks you through setting up a Telegram bot powered by Little Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Little Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.little/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Little itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Little Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_little_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Little Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Little Gateway...
...
# View live logs
journalctl --user -u little-gateway -f
...
little gateway start
little gateway stop
tail -f ~/.little/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `little gateway install` to update the plist.
:::
...
```bash
little gateway stop && little gateway start
```
...
little pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.little/.env`:
...
Control how much detail the bot shows when using tools. In `~/.little/config.yaml`:
...
Customize how the bot communicates by editing `~/.little/SOUL.md`:

For a full guide, see [Use SOUL.md with Little](/guides/use-soul-with-little).
...
```bash
# From the CLI
little cron list          # View all scheduled jobs
little cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u little-gateway` (Linux) or `~/.little/logs/gateway.log` (macOS) |
| Cron job output | `~/.little/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.little/cron/jobs.json` |
| Pairing data | `~/.little/pairing/` |
| Session history | `~/.little/sessions/` |
