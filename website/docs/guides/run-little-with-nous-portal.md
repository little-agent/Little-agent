...
Open [portal.little-agent.com/manage-subscription](https://portal.little-agent.com/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.little/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Little is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
little setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
little auth add nous --type oauth --manual-paste
# Then re-run `little setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Little Agent release notes" and summarize the top 3 hits.
```

You should see Little call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `little setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
little config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Little-4 for agent work

Little-4-70B and Little-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.little-agent.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Little Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.little-agent.com/info) carries this warning too — it's the official Nous guidance, not just a Little-side opinion.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Little will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
little cron create "every day at 9am" \
...
If you use [Little profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.little/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `little portal status` shows "not logged in" after `little setup --portal`
...
```bash
little config set model.provider nous
```
...
Re-verify with `little portal status`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Little doesn't replay it endlessly. Just log in again:

```bash
little auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/little-agent/Little-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`little portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Little profiles where you're using the wrong one (check `little profile current`)

### Want to revoke and start clean

```bash
little auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Little tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Little configurations
