...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `little setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Little spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.little/cron/output/` and you can read them anytime.
:::
...
Little will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Little Agent
```
...
Before creating cron jobs, ensure Little has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Little what you want:
...
Little will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Little to delegate each topic to a sub-agent:
...
Little will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
