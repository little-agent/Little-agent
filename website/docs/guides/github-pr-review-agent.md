...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Little Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Little instantly when PRs are opened or updated.
:::
...
- **Little Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  little gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.little/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Little can access GitHub. Start a chat:
...
```
Run: gh pr list --repo little-agent/Little-agent --state open --limit 3
```
...
Still in the chat, ask Little to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo little-agent/Little-agent
```
...
A skill gives Little consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.little/skills/code-review
```

Create `~/.little/skills/code-review/SKILL.md`:
...
Verify it loaded — start `little` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Little your team's standards:
...
2. Teach Little your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
little gateway status    # Is the gateway running?
little cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-little-plugin)** — wrap the review logic into a shareable plugin
