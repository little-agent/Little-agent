---
sidebar_position: 7
title: "Use SOUL.md with Little"
description: "How to use SOUL.md to shape Little Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Little

`SOUL.md` is the **primary identity** for your Little instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Little to feel like the same assistant every time you talk to it — or if you want to replace the Little persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Little should be
- what Little should avoid stylistically
- how Little should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Little is and how Little speaks
...
Little now uses only the global SOUL file for the current instance:

```text
~/.little/SOUL.md
```

If you run Little with a custom home directory, it becomes:
...
Little automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Little does not overwrite it
- if the file exists but is empty, Little adds nothing from it to the prompt

## How Little uses it

When Little starts a session, it reads `SOUL.md` from `LITTLE_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Little feels.
...
Little already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Little should sound.

# Avoid
What Little should not do.

# Defaults
How Little should behave when ambiguity appears.
```
...
Then restart Little or start a new session.
...
### I edited SOUL.md but Little still sounds the same

Check:
- you edited `~/.little/SOUL.md` or `$LITTLE_HOME/SOUL.md`
...
### Little is ignoring parts of my SOUL.md
