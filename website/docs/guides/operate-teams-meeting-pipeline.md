...
```bash
little teams-pipeline validate
```
...
```bash
little teams-pipeline token-health
little teams-pipeline token-health --force-refresh
```
...
```bash
little teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
little teams-pipeline maintain-subscriptions
little teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Little cron (recommended if you already run the Little gateway)

Little ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.little/scripts/`. First create the script:

```bash
mkdir -p ~/.little/scripts
cat > ~/.little/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec little teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.little/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
little cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/little-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Little Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=little
EnvironmentFile=/etc/little/env
ExecStart=/usr/local/bin/little teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/little-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Little Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now little-teams-pipeline-maintain.timer
systemctl list-timers little-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/little teams-pipeline maintain-subscriptions >> /var/log/little/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.little/.env` at the top of a wrapper script that crontab calls.
...
```bash
little teams-pipeline subscriptions   # should show expirationDateTime advanced
little teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
little teams-pipeline list
little teams-pipeline list --status failed
little teams-pipeline show <job-id>
```

### Replay a stored job

```bash
little teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
little teams-pipeline fetch --meeting-id <meeting-id>
little teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `little teams-pipeline maintain-subscriptions --dry-run`
- inspect `little teams-pipeline list --status failed`
...
- run `little teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `little teams-pipeline run`
...
- [ ] `little teams-pipeline validate` returns an OK snapshot
- [ ] `little teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Little cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
