...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use Little Agent with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

Little Agent supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
- **boto3** — install with `pip install little-agent[bedrock]`
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — Little detects the instance role automatically.
:::
...
pip install little-agent[bedrock]
...
Little auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `little model` picker shows these automatically, with recommended models at the top:
...
Bedrock works with all Little gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
Little checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. Little automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).

## One-Click AWS Deployment

For a fully automated deployment on EC2 with CloudFormation:

**[sample-little-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-little-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
