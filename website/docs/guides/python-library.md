---
sidebar_position: 5
title: "Using Little as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using Little as a Python Library

Little isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install Little directly from the repository:

```bash
pip install git+https://github.com/little-agent/Little-agent.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+https://github.com/little-agent/Little-agent.git
```

You can also pin it in your `requirements.txt`:

```text
little-agent @ git+https://github.com/little-agent/Little-agent.git
```

:::tip
The same environment variables used by the CLI are required when using Little as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Little is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding Little in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, Little includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your little config at runtime) |
