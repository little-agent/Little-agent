...
description: "How Little stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `little_cli/cron.py` | CLI `little cron` subcommands |
...
Jobs are stored in `~/.little/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `little cron` commands are run or during active CLI sessions.
...
```python
# ~/.little/scripts/check_competitors.py
...
2. **Environment variable** — `LITTLE_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.little/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `little cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `little cron` CLI provides direct job management:

```bash
little cron list                    # Show all jobs
little cron create                  # Interactive job creation (alias: add)
little cron edit <job_id>           # Edit job configuration
little cron pause <job_id>          # Pause a running job
little cron resume <job_id>         # Resume a paused job
little cron run <job_id>            # Trigger immediate execution
little cron remove <job_id>         # Delete a job
```
