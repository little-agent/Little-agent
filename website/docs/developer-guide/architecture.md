...
sidebar_position: 1
title: "Architecture"
description: "Little Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Little Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ little_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # LittleCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── little_state.py           # SQLite session/state database with FTS5
├── little_constants.py       # LITTLE_HOME, profile-aware paths
...
│
├── little_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `little` subcommands (large file)
...
│   ├── skills_config.py      # little skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # little tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # little gateway start/stop
│
...
```text
User input → LittleCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .little.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.little/plugins/` (user), `.little/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `little plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-little-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Little as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`little -p <name>`) gets its own LITTLE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
