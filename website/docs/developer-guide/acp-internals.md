...
The ACP adapter wraps Little' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
little acp / little-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.little/.env
  -> configure stderr logging
  -> construct LittleACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'little-agent[acp]==<version>' little-acp`, pointed at the `little-agent` PyPI release.
...
- `allow_once` -> Little `once`
- `allow_always` -> Little `always`
- reject options -> Little `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Little tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["little-acp"])
...
Instead it reuses Little' runtime resolver:

- `acp_adapter/auth.py`
- `little_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Little provider/credentials. It also always advertises a terminal setup auth method (`little-setup`, args `--setup`) so first-run registry clients can open Little' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.little/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `little-acp` toolset definition
- `little_cli/main.py` — `little acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `little-acp` script
