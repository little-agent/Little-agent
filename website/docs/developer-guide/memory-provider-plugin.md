...
description: "How to build a memory provider plugin for Little Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Little Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `little plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          little_home (str): Active LITTLE_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `little memory setup` | **Yes** |
| `save_config(values, little_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `little memory setup`:
...
All storage paths **must** use the `little_home` kwarg from `initialize()`, not hardcoded `~/.little`:

```python
# CORRECT — profile-scoped
from little_constants import get_little_home
data_dir = get_little_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.little/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `little my-provider status`, `little my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `little <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `little --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: little my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the little my-provider argparse tree.
