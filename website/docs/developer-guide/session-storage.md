...
Little Agent uses a SQLite database (`~/.little/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `little_state.py`


## Architecture Overview

```
~/.little/state.db (SQLite, WAL mode)
...
Multiple little processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.little/state.db
...
Default path: `~/.little/state.db`

This is derived from `little_constants.get_little_home()` which resolves to
`~/.little/` by default, or the value of `LITTLE_HOME` environment variable.
