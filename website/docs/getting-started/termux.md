...
description: "Run Little Agent directly on an Android phone with Termux"
---

# Little on Android with Termux

This is the tested path for running Little Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Little from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Little now ships a Termux-aware installer path:
...
- links `little` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `little` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/little" "$PREFIX/bin/little"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `little` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.little/.env`.
...
### `little doctor` says ripgrep or Node is missing
