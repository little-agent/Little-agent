...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Little Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Little setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Little Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Little Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Little for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Little working on my machine | `little setup` | Run a real chat and verify it responds |
| I already know my provider | `little model` | Save the config, then start chatting |
| I want a bot or always-on setup | `little gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `little model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `little model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Little cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Little Agent

**Option A — pip (simplest):**

```bash
pip install little-agent
little postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `little model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `little model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `little model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `little model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `little_cli/models.py` may change between releases) | `little model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `little model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `little model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `little model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `little model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `little model` (requires `copilot` CLI + `copilot login`) |
...
Little Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `little model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Little separates secrets from normal config:

- **Secrets and tokens** → `~/.little/.env`
- **Non-secret settings** → `~/.little/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
little config set model anthropic/claude-opus-4.6
little config set terminal.backend docker
little config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
little            # classic CLI
little --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Little ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `little` vs `little --tui`.
:::
...
```bash
little --continue    # Resume the most recent session
little -c            # Short form
```
...
```bash
little gateway setup    # Interactive platform configuration
```
...
- `little tools` — tune tool access per platform
- `little skills` — browse and install reusable workflows
...
```bash
little config set terminal.backend docker    # Docker isolation
little config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Little install directory (the curl installer placed it at
# ~/.little/little-agent on Linux/macOS or %LOCALAPPDATA%\little\little-agent on Windows):
...
```bash
little skills search kubernetes
...
(If you installed without `[all]`, run `cd ~/.little/little-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Little opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `little model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `little gateway setup` and check `little gateway status` |
| `little --continue` can't find old session | Switched profiles or session never saved | Check `little sessions list` and confirm you're in the right profile |
...
| `little doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `little model`
3. `little setup`
4. `little sessions list`
5. `little --continue`
6. `little gateway status`
...
| `little tools` | Configure which tools are enabled per platform |
| `little setup` | Full setup wizard (configures everything at once) |
| `little doctor` | Diagnose issues |
| `little update` | Update to latest version |
| `little gateway` | Start the messaging gateway |
| `little --continue` | Resume last session |
