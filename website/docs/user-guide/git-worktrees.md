...
title: "Git Worktrees"
description: "Run multiple Little agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Little Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Little so each session has a clean, isolated working directory.

## Why Use Worktrees with Little?

Little treats the **current working directory** as the project root:

- CLI: the directory where you run `little` or `little chat`
...
git worktree add ../repo-feature feature/little-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/little-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Little there:

```bash
cd ../repo-feature

# Start Little in the worktree
little
```
...
git worktree add ../repo-experiment-a feature/little-a
git worktree add ../repo-experiment-b feature/little-b
```
...
- Works on its own branch (`feature/little-a` vs `feature/little-b`).
...
- Little checkpoint data under `~/.little/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Little experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/little-checkpoints-docs`, `feature/little-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Little from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `little -w` (Automatic Worktree Mode)

Little has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `little/little-<hash>`).
...
```bash
little -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `little -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Little session its own clean checkout.
