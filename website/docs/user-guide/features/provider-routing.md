...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Little Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.little/config.yaml`:
...
- **CLI mode** — configured in `~/.little/config.yaml`, loaded at startup
