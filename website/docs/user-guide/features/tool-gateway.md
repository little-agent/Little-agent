...
The Tool Gateway is included with every paid [Nous Portal](https://portal.little-agent.com) subscription. It routes Little' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.

<div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap', margin: '1.5rem 0'}}>
  <a href="https://portal.little-agent.com/manage-subscription" style={{background: 'var(--ifm-color-primary)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold'}}>Start or manage subscription →</a>
</div>
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Little default to FLUX 2 Klein. |
...
```bash
little setup --portal     # Nous OAuth, set Nous as provider, and turn on the Tool Gateway in one go
```

Already have Little configured? Just switch your provider:

```bash
little model              # Pick Nous Portal — Little will offer to turn on the Tool Gateway
```

When you select Nous Portal, Little offers to turn on the Tool Gateway. Accept, and you're done — every supported tool is live on the next run.

Check what's active at any time:

```bash
little portal status      # Portal auth + Tool Gateway routing summary
little portal tools       # Gateway catalog with current routing per tool
little status             # Full system status (Tool Gateway is one section)
```

`little portal status` shows a section like:
...
The Tool Gateway is a **paid-subscription** feature. Free-tier Nous accounts can use Portal for inference but don't include managed tools — [upgrade your plan](https://portal.little-agent.com/manage-subscription) to unlock the gateway.
...
```bash
little tools          # Interactive picker for each tool category
```
...
The set evolves — `little tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `little model` and `little tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # Little now uses FIRECRAWL_API_KEY from .env
```

`little tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.little/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `little tools`. Little shows a clear error pointing at the portal.

### Can I see usage or costs per tool?

Yes — the [Nous Portal dashboard](https://portal.little-agent.com) breaks usage down by tool so you can see what's driving your bill.

### Is Modal (serverless terminal) included?

Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `little setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, Little skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
