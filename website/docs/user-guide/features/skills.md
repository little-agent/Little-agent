...
All skills live in **`~/.little/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Little at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Little to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.little/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
little chat --toolsets skills -q "What skills do you have?"
little chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.little/cache/chart-q4-2025.png
...
When a missing value is encountered, Little asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `little setup` or `~/.little/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `little config migrate` prompts for unconfigured settings, and `little config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.little/skills/                  # Single source of truth
...
If you maintain skills outside of Little — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Little to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.little/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.little/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Little process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Little ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.little/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.little/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `little bundles list`.
...
little bundles create research

# Overwrite an existing bundle
little bundles create backend-dev --skill ... --force

# Delete a bundle
little bundles delete backend-dev

# Re-scan ~/.little/skill-bundles/ and report changes
little bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.little/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.little/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
little skills browse                              # Browse all hub skills (official first)
little skills browse --source official            # Browse only official optional skills
little skills search kubernetes                   # Search all sources
little skills search react --source skills-sh     # Search the skills.sh directory
little skills search https://mintlify.com/docs --source well-known
little skills inspect openai/skills/k8s           # Preview before installing
little skills install openai/skills/k8s           # Install with security scan
little skills install official/security/1password
little skills install skills-sh/vercel-labs/json-render/json-render-react --force
little skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
little skills list --source hub                   # List hub-installed skills
little skills check                               # Check installed hub skills for upstream updates
little skills update                              # Reinstall hub skills with upstream changes when needed
little skills audit                               # Re-scan all hub skills for security
little skills uninstall k8s                       # Remove a hub skill
little skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
little skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
little skills publish skills/my-skill --to github --repo owner/repo
little skills snapshot export setup.json          # Export skill config
little skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Little. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `little skills search <query> --source skills-sh`. Little resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Little currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Little repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Little can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
little skills search https://mintlify.com/docs --source well-known
little skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
little skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Little can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
little skills install openai/skills/k8s
little skills tap add myorg/skills-repo
```
...
- Little source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Little supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Little source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Little can search and convert agent entries from LobeHub's public catalog into installable Little skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Little source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Little integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Little' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Little source id: `browse-sh`
- Trust level: `community`

```bash
little skills search airbnb --source browse-sh
little skills inspect browse-sh/airbnb.com/search-listings-ddgioa
little skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Little fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Little source id: `url`
...
```bash
little skills install https://sharethis.chat/SKILL.md
little skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
little skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `little skills update` re-fetches from the same URL automatically when you want to refresh.
...
`little skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Little | Always trusted |
...
```bash
little skills check          # Report which installed hub skills changed upstream
little skills update         # Reinstall only the skills with updates available
little skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Little users add with `little skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.little.tags`, `version`, `author`, `platforms`, `metadata.little.config`).
...
Little discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/little-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Little user can subscribe and install:

```bash
little skills tap add my-org/little-skills
little skills search deploy
little skills install my-org/little-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.little/.hub/taps.json`:
...
The `little skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `little skills tap list` shows the effective path per tap.
...
```bash
little skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.little/.hub/taps.json` (created on demand).

## Bundled skill updates (`little skills reset`)

Little ships with a set of bundled skills in `skills/` inside the repo. On install and on every `little update`, a sync pass copies those into `~/.little/skills/` and records a manifest at `~/.little/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Little recomputes the hash of your local copy and compares it to the origin hash:
...
little skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `LITTLE_HOME`, so `little -p coder skills reset <name>` only affects that profile.
:::
