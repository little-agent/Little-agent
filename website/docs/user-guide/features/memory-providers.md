...
Little Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
little memory setup      # interactive picker + configuration
little memory status     # check what's active
little memory off        # disable external provider
```

You can also select the active memory provider via `little plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.little/config.yaml`:
...
When a memory provider is active, Little automatically:
...
**Setup Wizard:**
```bash
little memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `little honcho setup` command still works (it now redirects to `little memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$LITTLE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$LITTLE_HOME/honcho.json` > `~/.little/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/little-ai/little-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/little).
...
:::tip Migrating from `little honcho`
If you previously used `little honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Little profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Little profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Little profile. Host key `little` → default; `little.<profile>` for others. |
...
```bash
little profile create coder --clone
```

`--clone` creates a `little.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Little profile, creates host blocks for any profile without one, inherits settings from the default `little` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/little-ai/little-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/little).
...
little memory setup    # select "openviking"
# Or manually:
little config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.little/.env
```
...
**Config:** `$LITTLE_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `little-user` | User identifier |
...
**Setup:**
```bash
little memory setup    # select "hindsight"
# Or manually:
little config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.little/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p little ui start`

**Config:** `$LITTLE_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `little` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Little Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/little-agent/Little-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
little memory setup    # select "holographic"
# Or manually:
little config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.little-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$LITTLE_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
little memory setup    # select "retaindb"
# Or manually:
little config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.little/.env
```
...
# Then configure Little
little memory setup    # select "byterover"
# Or manually:
little config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$LITTLE_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
little memory setup    # select "supermemory"
# Or manually:
little config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.little/.env
```

**Config:** `$LITTLE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `little` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `little-{identity}` → `little-coder`) to isolate memories per Little profile
...
- **Local storage providers** (Holographic, ByteRover) use `$LITTLE_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$LITTLE_HOME/` so each profile has its own credentials
