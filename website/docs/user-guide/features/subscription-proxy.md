...
chat completions — use your Little-managed provider subscription as their
...
| Use case | "Use Little as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Little stores
the refresh token in `~/.little/auth.json` — the same place all Little
...
Model:      Little-4-70B    # or Little-4.3-36B, Little-4-405B
```
...
interface in `little_cli/proxy/adapters/`.
...
If you see `not logged in`, run `little auth add nous`. If you see
...
supports; check `portal.little-agent.com/models`.
...
[portal.little-agent.com](https://portal.little-agent.com).
...
`little_cli/proxy/adapters/<provider>.py` and registering it in
