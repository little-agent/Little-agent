...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Little's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Little instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
little memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$LITTLE_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `little` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Little syncs them back at session init.
...
The `little honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `little memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
little honcho status          # Connection status, config, and key settings
little honcho setup           # Redirects to `little memory setup`
little honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
little honcho peer            # Show or update peer names + dialectic reasoning level
little honcho mode            # Show or set recall mode (hybrid/context/tools)
little honcho tokens          # Show or set token budget for context and dialectic
little honcho identity        # Seed or show the AI peer's Honcho identity
little honcho sync            # Sync Honcho config to all existing profiles
little honcho peers           # Show peer identities across all profiles
little honcho sessions        # List known Honcho session mappings
little honcho map             # Map current directory to a Honcho session name
little honcho enable          # Enable Honcho for the active profile
little honcho disable         # Disable Honcho for the active profile
little honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `little honcho`

If you previously used the standalone `little honcho setup`:
...
No re-login or re-setup needed. Run `little memory setup` and select "honcho" — the wizard detects your existing config.
