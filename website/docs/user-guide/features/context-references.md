...
Type `@` followed by a reference to inject content directly into your message. Little expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Little env: `$LITTLE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LITTLE_HOME/skills/.hub/`
