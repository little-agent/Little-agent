...
description: "Customize Little Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Little Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `LITTLE_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Little is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Little now seeds a default `SOUL.md` automatically in:

```text
~/.little/SOUL.md
```

More precisely, it uses the current instance's `LITTLE_HOME`, so if you run Little with a custom home directory, it will use:
...
- Little creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Little loads `SOUL.md` only from `LITTLE_HOME`
- Little does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Little falls back to a built-in default identity
...
If Little loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `LITTLE_HOME`, the personality belongs to the Little instance itself.

That also makes it easier to teach users:
- "Edit `~/.little/SOUL.md` to change Little' default personality."
...
- how Little should handle uncertainty, disagreement, or ambiguity
...
## What Little injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Little falls back to a built-in default identity ("You are Little Agent, an intelligent AI assistant created by Little Agent Team..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Little ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Little, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Little its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.little/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.little/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-little)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Little speaks
- `display.skin` and `/skin` affect how Little looks in the terminal
