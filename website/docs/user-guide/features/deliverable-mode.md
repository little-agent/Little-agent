...
When Little Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.little/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.little/config.yaml` (switchable per session
via `/personality`).
...
If you use Little' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.little/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Little Agent's deliverable
