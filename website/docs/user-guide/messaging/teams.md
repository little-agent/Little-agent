...
description: "Set up Little Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Little Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `little gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Little strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create little-bot --allow-anonymous
devtunnel port create little-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host little-bot
...
```bash
LITTLE_UID=$(id -u) LITTLE_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.little/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.little/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create little-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs little` for tracebacks |
...
- Store credentials in `~/.little/.env` with permissions `600` (`chmod 600 ~/.little/.env`)
