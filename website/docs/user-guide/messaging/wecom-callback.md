...
Connect Little to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Little supports two WeCom integration modes:
...
> Run `little gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Little decrypts the message, queues it for the agent
...
(Use `little gateway start` only after `little gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Little and the token the
...
in `~/.little/.env` values around `=` will also break signature checks. After
fixing, restart `little gateway run`.
...
Check `little gateway run` logs for the bound host/port. If the adapter bound to
