...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Little Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Little to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.little/.env`:
...
All standard Little commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.little/logs/gateway.log`
...
4. Consider enabling verbose logging: `LITTLE_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Little automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
LITTLE_LOG_LEVEL=debug little gateway
```
...
```bash
little chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
