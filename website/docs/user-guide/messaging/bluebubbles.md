...
Connect Little to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Little and the BlueBubbles server
...
Or set environment variables directly in `~/.little/.env`:
...
When someone messages your iMessage, Little automatically sends them a pairing code. Approve it with:
```bash
little pairing approve bluebubbles <CODE>
```
Use `little pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.little/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.little/.env`):
```bash
...
Little will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Little
Little → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Little sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.little/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Little resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `little logs gateway` for webhook errors (or `little logs -f` to follow in real-time)
