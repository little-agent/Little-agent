...
sidebar_position: 12
title: "Google Chat"
description: "Set up Little Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Little Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Little
...
> Run `little gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Little can read (e.g.,
`~/.little/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a little restart)
...
- Principal: `little-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Little calls
...
- **App name**: whatever you want users to see ("Little" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/little-chat-events`.
...
`ADDED_TO_SPACE` event that Little uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Little

Add the Google Chat section to `~/.little/.env`:
...
Install the dependencies the Google Chat adapter needs (no Little extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Little is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Little detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Little session.
...
3. Download the JSON. Move it onto the host that runs Little.
4. On the host, register the client with Little:
...
That writes `~/.little/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.little/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.little/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Little isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `little gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Little will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.little/google_chat_user_tokens/<sanitized_email>.json` (filesystem
