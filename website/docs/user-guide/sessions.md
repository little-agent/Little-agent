...
Little Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.little/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Little stores session history so it can resume conversations, but it does not
...
Little explicitly injects for that turn.
...
For example, if a user sends an image and asks Little to make a meme from it,
Little may inspect that image once with vision and run an image-processing
...
`little sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`little` or `little chat`) |
...
little -c "my project"   # → resumes "my project #3"
```
...
little --resume 20250305_091523_a1b2c3d4
little -r 20250305_091523_a1b2c3d4

# Resume by title
little --resume "refactoring auth"

# Or with the chat subcommand
little chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `little sessions list`.

### Conversation Recap on Resume

When you resume a session, Little displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Little session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.little/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `little -r "<title>"` from the shell) and pick up where the platform left off.
...
Little automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `little sessions list` or `little sessions browse`.
...
```bash
little sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Little creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`little -c "my project"`), it automatically picks the most recent session in the lineage.
...
Little provides a full set of session management commands via `little sessions`:
...
# Filter by platform
little sessions list --source telegram

# Show more sessions
little sessions list --limit 50
```
...
little sessions export backup.jsonl

# Export sessions from a specific platform
little sessions export telegram-history.jsonl --source telegram

# Export a single session
little sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
little sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
little sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
little sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
little sessions prune

# Custom age threshold
little sessions prune --older-than 30

# Only prune sessions from a specific platform
little sessions prune --source telegram --older-than 60

# Skip confirmation
little sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`little insights`](/reference/cli-commands#little-insights).
...
When Little cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Little uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Little in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.little/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.little/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.little/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.little/sessions/`. They are no longer written or
read by Little. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Little process in the same `LITTLE_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.little/config.yaml`:
...
little sessions delete <session_id>
