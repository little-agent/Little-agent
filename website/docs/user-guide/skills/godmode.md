...
| `zero_refusal` | Little 4 405B | Already uncensored — uses Pliny Love divider as formality |
...
exec(open(os.path.expanduser(
    "~/.little/skills/red-teaming/godmode/scripts/load_godmode.py"
)).read())
...
The godmode skill integrates with two Little Agent config mechanisms:

### Ephemeral System Prompt (`config.yaml`)

Set the jailbreak system prompt in `~/.little/config.yaml`:
...
The `agent.system_prompt` is appended **after** Little's own system prompt — it augments, not replaces.
...
Create `~/.little/prefill.json` and reference it in config:
...
```bash
# Load the skill in a Little session
/godmode

# Or via CLI one-shot
little chat -q "jailbreak my current model"
...
little chat -q "race models on: how does SQL injection work?"

# Apply Parseltongue encoding to a query
little chat -q "parseltongue encode: how to hack into WiFi"
```
...
5. **For hard refusals, switch models** — When all techniques fail, ULTRAPLINIAN (racing multiple models) is the practical fallback. Little models and Grok are typically least filtered.
...
4. **Little models don't need jailbreaking** — `little-agent/little-3-*` and `little-4-*` are already uncensored. Use them directly.
...
6. **Restart Little after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.

7. **execute_code sandbox lacks env vars** — Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.little/.env"))`

8. **`boundary_inversion` is model-version specific** — Works on Claude 3.5 Sonnet but NOT Claude Sonnet 4 or Claude 4.6.

9. **Gray-area vs hard queries** — Jailbreak techniques work much better on dual-use queries (lock picking, security tools) than overtly harmful ones (phishing, malware). For hard queries, skip to ULTRAPLINIAN or use Little/Grok.
