...
| | |
|---|---|
| Source | Optional — install with `little skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Little Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.little/.env`:
...
```bash
CANVAS="python $LITTLE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
