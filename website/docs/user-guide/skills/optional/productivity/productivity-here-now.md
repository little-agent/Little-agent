...
| | |
|---|---|
| Source | Optional — install with `little skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LITTLE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LITTLE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LITTLE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client little
```
...
```bash
PUBLISH="${LITTLE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client little
```
...
```bash
DRIVE="${LITTLE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `little`)    |
