---
title: "Telephony — Give Little phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Little phone capabilities without core tool changes"
---
...
Give Little phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `little skills install official/productivity/telephony` |
| Path | `optional-skills/productivity/telephony` |
| Version | `1.0.0` |
| Author | Little Agent Team |
| License | MIT |
...
This optional skill gives Little practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.little/.env`
...
It does **not** turn Little into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Little memory
...
### 1) "I want Little to own a real phone number"
Use **Twilio**.
...
- pairs well with Little `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.little/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
little skills search telephony
little skills install official/productivity/telephony
```
...
Then save credentials into Little:
...
3. Buy it and save it into `~/.little/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Little calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Little's existing `text_to_speech` support.

Use this when:
- you want the call to use Little's configured TTS voice rather than Twilio `<Say>`
...
Recommended Little TTS -> Twilio Play workflow:

1. Generate the audio with Little `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Little to create the message audio with `text_to_speech`
...
Important note:
- Little TTS is great for prerecorded outbound messages
...
- Little STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Little memory.
...
- Do not store arbitrary third-party phone numbers in Little memory.
...
3. persist that number to `~/.little/.env`
