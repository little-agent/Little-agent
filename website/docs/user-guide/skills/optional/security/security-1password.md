...
| | |
|---|---|
| Source | Optional — install with `little skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Little Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Little terminal calls (desktop app flow only)
...
### Service Account (recommended for Little)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.little/.env` (the skill will prompt for this on first load).
...
## Little Execution Pattern (desktop app flow)

Little terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/little-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/little-op.sock"
