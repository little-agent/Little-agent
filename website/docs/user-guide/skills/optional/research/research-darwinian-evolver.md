...
| | |
|---|---|
| Source | Optional — install with `little skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Little Agent |
| License | MIT |
...
import upstream classes into Little itself.
...
```bash
mkdir -p ~/.little/cache/darwinian-evolver && cd ~/.little/cache/darwinian-evolver
...
```bash
cd ~/.little/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.little/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.little/skills/research/darwinian-evolver
DE_DIR=~/.little/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Little core.
   Custom driver scripts under `~/.little/skills/...` are user-side and fine.
