---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Little Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Little Agent"
---
...
Migrate a user's OpenClaw customization footprint into Little Agent. Imports Little-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Related skills | [`little-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-little-agent) |
...
# OpenClaw -> Little Migration

Use this skill when a user wants to move their OpenClaw setup into Little Agent with minimal manual cleanup.
...
```bash
little claw migrate              # Full interactive migration
...
little claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `little setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_little.py` to:

- import `SOUL.md` into the Little home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Little memory entries
- merge OpenClaw command approval patterns into Little `command_allowlist`
- migrate Little-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.little/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Little workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.little/tts/`
- archive non-secret docs that do not have a direct Little destination
...
- `scripts/openclaw_to_little.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py`

Do not guess a shorter path like `~/.little/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.little/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Little-compatible secrets, currently:
...
Little CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Little. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Little workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Little skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.little/skills/migration/openclaw-migration/scripts/openclaw_to_little.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Little unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Little targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Little destination. Unsupported auth blobs must still be reported as skipped.
...
- Little persona state imported
- Little memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.little/skills/openclaw-imports/`
