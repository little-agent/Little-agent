...
description: "Configure and use Honcho memory with Little -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Little -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `little skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Little

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Little profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/little#running-honcho-locally-with-little

### Verify

```bash
little honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Little profile's AI peer
...
Honcho models conversations as interactions between **peers**. Little creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Little instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Little run |
| `global` | Single session across all directories |

Manual override: `little honcho map my-project-name`
...
Each Little profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block little.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `little.coder` host block in `honcho.json`
...
```bash
little honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Little when Honcho memory is active.
...
Config file: `$LITTLE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `little honcho setup`. Ensure `memory.provider: honcho` is in `~/.little/config.yaml`.

### Memory not persisting across sessions
Check `little honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `little profile create <name> --clone`. For existing profiles: `little honcho sync`.
...
| Command | Description |
|---------|-------------|
| `little honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `little honcho status` | Show resolved config, connection test, peer info for active profile |
| `little honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `little honcho disable` | Disable Honcho for the active profile |
| `little honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `little honcho peers` | Show peer identities across all profiles |
| `little honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `little honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `little honcho sessions` | List known directory-to-session-name mappings |
| `little honcho map <name>` | Map current working directory to a Honcho session name |
| `little honcho identity` | Seed AI peer identity or show both peer representations |
| `little honcho sync` | Create host blocks for all Little profiles that don't have one yet |
| `little honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Little + Honcho |
| `little memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `little memory status` | Show active memory provider and config |
| `little memory off` | Disable external memory provider |
