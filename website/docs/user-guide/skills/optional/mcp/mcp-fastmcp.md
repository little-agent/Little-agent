...
| | |
|---|---|
| Source | Optional — install with `little skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Little or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Little. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.little/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.little/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Little integration, either:

- configure the server in `~/.little/config.yaml` using the `native-mcp` skill, or
...
### Little cannot see the deployed server

The server-building part may be correct while the Little config is not. Load the `native-mcp` skill and configure the server in `~/.little/config.yaml`, then restart Little.
