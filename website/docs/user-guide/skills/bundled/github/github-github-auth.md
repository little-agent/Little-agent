...
- Give it a name like "little-agent"
- Select scopes:
...
- Give it a title like "little-agent-&lt;machine-name>"
...
elif [ -f ~/.little/.env ] && grep -q "^GITHUB_TOKEN=" ~/.little/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.little/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
