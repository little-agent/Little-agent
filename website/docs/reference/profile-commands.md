...
This page covers all commands related to [Little profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `little profile` without a subcommand shows help.
...
```bash
little profile use <name>
```

Sets `<name>` as the active profile. All subsequent `little` commands (without `-p`) will use this profile.
...
little profile use default
```

## `little profile create`

```bash
little profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `little profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `little update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
little profile create work2 --clone --clone-from work
```

## `little profile describe`

```bash
little profile describe [<name>] [options]
```
...
little profile describe researcher

# Set it explicitly
little profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
little profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
little profile describe --all --auto
```

## `little profile delete`

```bash
little profile delete <name> [options]
```
...
```bash
little profile delete mybot
little profile delete mybot --yes
```
...
```bash
little profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Little home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ little profile show work
Profile: work
Path:    ~/.little/profiles/work
...
## `little profile alias`

```bash
little profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Little installation.
...
```bash
little profile alias work
# Creates/updates ~/.local/bin/work

little profile alias work --name mywork
# Creates ~/.local/bin/mywork

little profile alias work --remove
# Removes the wrapper script
```

## `little profile rename`

```bash
little profile rename <old-name> <new-name>
```
...
```bash
little profile rename mybot assistant
# ~/.little/profiles/mybot → ~/.little/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `little profile export`

```bash
little profile export <name> [options]
```
...
```bash
little profile export work
# Creates work.tar.gz in the current directory

little profile export work -o ./work-2026-03-29.tar.gz
```

## `little profile import`

```bash
little profile import <archive> [options]
```
...
```bash
little profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

little profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`little profile export` / `import` are still the right commands for
...
### `little profile install`

```bash
little profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `little -p telemetry`). |
...
little profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
little profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
little profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
little profile install ./telemetry/
```

### `little profile update`

```bash
little profile update <name> [--force-config] [--yes]
```
...
```bash
little profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Little version, author, env var requirements, the source URL/path, and
...
`little profile list` also shows the distribution name and version in a
`Distribution` column, and `little profile show <name>` / `delete <name>`
...
little profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
little profile install https://github.com/your-org/internal-assistant.git
```
...
`little_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Little version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Little can clone from.
3. Tell recipients to run `little profile install <your-repo-url>`.
...
## `little -p` / `little --profile`

```bash
little -p <name> <command> [options]
little --profile <name> <command> [options]
```

Global flag to run any Little command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
little -p work chat -q "Check the server status"
little --profile dev gateway start
little -p personal skills list
little -p work config edit
```

## `little completion`

```bash
little completion <shell>
```
...
- `little profile <TAB>` — subcommands (list, use, create, etc.)
- `little profile use <TAB>` — profile names
- `little -p <TAB>` — profile names
