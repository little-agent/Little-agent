...
Maps each role archetype to the Little skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public little-agent
...
## Little skills relevant to video production

### Visual / rendering skills (`little-agent/skills/creative/`)
...
### Design / pre-production skills (`little-agent/skills/creative/`)
...
### Audio / media skills (`little-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`little-agent/skills/devops/`)
...
These are **not** Little skills but external CLIs / APIs that profiles invoke.
...
## Built-in Little tools for media review

These are native Little tools — not invoked via terminal but through their own
...
key is present in `~/.little/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Little profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.little/.env` → environment variable.
...
When a new Little-public video skill ships:
