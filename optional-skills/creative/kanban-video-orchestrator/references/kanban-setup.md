...
the actual `setup.sh` that creates the project workspace, configures Little
...
> [little-agent/kanban-video-pipeline](https://github.com/little-agent/kanban-video-pipeline).
...
2. **Create profiles** — `little profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.little/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `little kanban create` assigned to the director
...
```bash
little profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.little/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.little/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Little
...
Each profile gets a `SOUL.md` at `~/.little/profiles/<name>/SOUL.md` that
...
```bash
little kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.little/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.little/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.little/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.little/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY little ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY little OPENROUTER_API_KEY || exit 1
# ...
```
