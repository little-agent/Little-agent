...
little kanban watch --tenant <project-slug>

# Snapshot of the board
little kanban list --tenant <project-slug>
little kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
little kanban stats --tenant <project-slug>

# Visual dashboard (browser)
little dashboard

# Inspect a specific task (includes comments + events)
little kanban show <task-id>

# Follow a single task's event stream
little kanban tail <task-id>
```

Verify available subcommands with `little kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `little kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `little kanban block <id>` then `little kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `little kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `little kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `little profile list` (profile exists?), `little status` (gateway/dispatcher up?), and verify tenant. |
...
little kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
little kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(little kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
little kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
little kanban block <task-id>
little kanban archive <task-id>

# Diagnose what happened
little kanban show <task-id>      # task body, comments, recent events
little kanban tail <task-id>      # follow the live event stream
little kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `little kanban create` for the director
...
while true; do
    clear
    little kanban list --tenant <slug>
    echo "---"
    little kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `little kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `little profile list` and start
