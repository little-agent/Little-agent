#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `little kanban list` and
...
def little_available() -> bool:
    return shutil.which("little") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed little CLI."""
    try:
        out = subprocess.run(
            ["little", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `little kanban list`
    out = subprocess.run(
        ["little", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["little", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not little_available():
        print("ERROR: 'little' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
