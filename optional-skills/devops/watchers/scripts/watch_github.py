...
Usage (via cron with --no-agent):
    ...
    little cron create little-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$LITTLE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name little-issues --repo little-agent/Little-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.little/.env to avoid the 60 req/hr
