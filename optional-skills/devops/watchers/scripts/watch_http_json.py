...
Usage (via cron with --no-agent):
    ...
    little cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LITTLE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Little-Watcher/1.0"})
