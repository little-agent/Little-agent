"""
Little MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Little-specific extra)

Usage:
    little mcp serve
    little mcp serve --verbose
...
logger = logging.getLogger("little.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LITTLE_HOME."""
    try:
        from little_constants import get_little_home
        return get_little_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LITTLE_HOME", Path.home() / ".little")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from little_constants import get_little_home
        directory_file = get_little_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LITTLE_HOME", Path.home() / ".little")
...
class EventBridge:
    ...
    This is the Little equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from little_constants import get_little_home
            db_file = get_little_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LITTLE_HOME", Path.home() / ".little")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Little MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "little",
        instructions=(
            "Little Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Little MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
