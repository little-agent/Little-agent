...
## Client ID
- Need to register a "Desktop app" OAuth client on a Little Agent Team GCP project
...
## Token Lifecycle
- Store at `~/.little/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `little_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `little_cli/models.py` — Add Gemini model catalog
4. `little_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `little_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `little_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Little Agent Team GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via LITTLE_GEMINI_CLIENT_ID env var
...
- little-agent Copilot OAuth: `little_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
