"""Persistent slash-command worker — one LittleCLI per TUI session.
...
def _run(cli: LittleCLI, command: str) -> str:
...
def main():
    ...
    os.environ["LITTLE_SESSION_KEY"] = args.session_key
    os.environ["LITTLE_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = LittleCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
