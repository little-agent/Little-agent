"""Windows UTF-8 bootstrap for Little entry points.
...
should be imported at the very top of every Little entry point
(``little``, ``little-agent``, ``little-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import little_bootstrap``
# (or ``from little_bootstrap import apply_windows_utf8_bootstrap``) at
