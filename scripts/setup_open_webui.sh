...
# Bootstrap Open WebUI against Little Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.little/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-little.sh
...
#   LITTLE_API_PORT=8642
#   LITTLE_API_HOST=127.0.0.1
#   LITTLE_API_MODEL_NAME='Little Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Little Agent WebUI}"
...
LITTLE_ENV_FILE="${LITTLE_ENV_FILE:-$HOME/.little/.env}"
LITTLE_API_PORT="${LITTLE_API_PORT:-8642}"
LITTLE_API_HOST="${LITTLE_API_HOST:-127.0.0.1}"
LITTLE_API_CONNECT_HOST="${LITTLE_API_CONNECT_HOST:-127.0.0.1}"
LITTLE_API_MODEL_NAME="${LITTLE_API_MODEL_NAME:-Little Agent}"
LITTLE_API_BASE_URL="http://${LITTLE_API_CONNECT_HOST}:${LITTLE_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-little.sh"
LOG_DIR="$HOME/.little/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$LITTLE_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$LITTLE_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.little.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.little</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.little"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.little"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-little.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Little Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-little.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.little/logs/openwebui.log
StandardError=append:%h/.little/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-little.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$LITTLE_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Little API server is configured...'
  upsert_env API_SERVER_ENABLED true "$LITTLE_ENV_FILE"
  upsert_env API_SERVER_HOST "$LITTLE_API_HOST" "$LITTLE_ENV_FILE"
  upsert_env API_SERVER_PORT "$LITTLE_API_PORT" "$LITTLE_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$LITTLE_API_MODEL_NAME" "$LITTLE_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$LITTLE_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Little gateway so API server settings take effect...'
  little gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${LITTLE_API_CONNECT_HOST}:${LITTLE_API_PORT}/health" >/dev/null; then
    log 'Little API server did not answer on the first check. Trying to start gateway in the background...'
    nohup little gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${LITTLE_API_CONNECT_HOST}:${LITTLE_API_PORT}/health" >/dev/null
  ...
  log "Little API endpoint: ${LITTLE_API_BASE_URL}"
