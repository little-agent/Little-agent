...
# Python-driven tool session (e.g. Little terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:little-agent/Little-agent.git"
REPO_URL_HTTPS="https://github.com/little-agent/Little-agent.git"
LITTLE_HOME="${LITTLE_HOME:-$HOME/.little}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${LITTLE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$LITTLE_INSTALL_DIR"
...
#   code at /usr/local/lib/little-agent, command at /usr/local/bin/little,
#   data still at /root/.little (LITTLE_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.little/little-agent"
            echo "                   default (root, Linux): /usr/local/lib/little-agent"
            echo "  --little-home PATH  Data directory (default: ~/.little, or \$LITTLE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Little installs the code under"
            echo "  /usr/local/lib/little-agent and links the command into"
            echo "  /usr/local/bin/little (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$LITTLE_HOME"
            echo "  (default /root/.little).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$LITTLE_HOME/little-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs little setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `little` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $LITTLE_HOME/little-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $LITTLE_HOME/little-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/little-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $LITTLE_HOME/little-agent — then preserve it)
#
# Always no-op when the user set --dir or $LITTLE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in LITTLE_HOME.
    if is_termux; then
        INSTALL_DIR="$LITTLE_HOME/little-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$LITTLE_HOME/little-agent/.git" ]; then
            INSTALL_DIR="$LITTLE_HOME/little-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/little-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/little-agent"
        ...
        # /usr/local/bin/little wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/little"
        log_info "  Data:    $LITTLE_HOME (unchanged)"
...
get_little_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/little" ]; then
        echo "$link_dir/little"
    else
        echo "little"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/little-agent/Little-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/little-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/little-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$LITTLE_HOME/node/bin/node" ]; then
        export PATH="$LITTLE_HOME/node/bin:$PATH"
        local found_ver=$("$LITTLE_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Little-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.little/node/..."
    ...
    # Place into ~/.little/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$LITTLE_HOME/node"
    mkdir -p "$LITTLE_HOME"
    mv "$extracted_dir" "$LITTLE_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$LITTLE_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$LITTLE_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$LITTLE_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$LITTLE_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$LITTLE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.little/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Little install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Little Agent itself does not require or retain root access."
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"little-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up little command..."

    if [ "$USE_VENV" = true ]; then
        LITTLE_BIN="$INSTALL_DIR/venv/bin/little"
    else
        LITTLE_BIN="$(which little 2>/dev/null || echo "")"
        if [ -z "$LITTLE_BIN" ]; then
            log_warn "little not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$LITTLE_BIN" ]; then
        log_warn "little entry point not found at $LITTLE_BIN"
    ...
    # Create a user-facing shim for the little command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $LITTLE_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$LITTLE_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/little"
    cat > "$command_link_dir/little" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$LITTLE_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/little"
    log_success "Installed little launcher → $command_link_display_dir/little"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "little command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v little' \
                >/dev/null 2>&1; then
        ...
        log_info "little not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Little Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Little Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Little Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so little works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.little directory structure (config at top level, code in subdir)
    mkdir -p "$LITTLE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.little/.env (top level, easy to find)
    if [ ! -f "$LITTLE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$LITTLE_HOME/.env"
            log_success "Created ~/.little/.env from template"
        else
            touch "$LITTLE_HOME/.env"
            log_success "Created ~/.little/.env"
        fi
    else
        log_info "~/.little/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$LITTLE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.little/config.yaml (top level, easy to find)
    if [ ! -f "$LITTLE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$LITTLE_HOME/config.yaml"
            log_success "Created ~/.little/config.yaml from template"
        fi
    else
        log_info "~/.little/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$LITTLE_HOME/SOUL.md" ]; then
        cat > "$LITTLE_HOME/SOUL.md" << 'SOUL_EOF'
# Little Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Little communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.little/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.little/"

    # Seed bundled skills into ~/.little/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.little/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.little/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$LITTLE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$LITTLE_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.little/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Little Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Little will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (little --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'little setup' after install."
        return 0
    fi
    ...
    # Run little setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m little_cli.main setup < /dev/tty
    else
        python -m little_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$LITTLE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Little to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$LITTLE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'little whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                LITTLE_CMD="$(get_little_command_path)"
                $LITTLE_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'little whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'little gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $LITTLE_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $LITTLE_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: little gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: little gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $LITTLE_CMD gateway > "$LITTLE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.little/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: little gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: little gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $LITTLE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $LITTLE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $LITTLE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}little${NC}              Start chatting"
    echo -e "   ${GREEN}little setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}little config${NC}       View/edit configuration"
    echo -e "   ${GREEN}little config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}little gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}little update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'little' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'little' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'little' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$LITTLE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$LITTLE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$LITTLE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$LITTLE_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$LITTLE_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Little for pip install"
    ...
    LITTLE_CMD="$(command -v little 2>/dev/null || echo "")"
    if [ -n "$LITTLE_CMD" ]; then
        log_info "Running little setup..."
        "$LITTLE_CMD" setup
    else
        log_warn "little command not found on PATH"
        log_info "Try: python -m little_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$LITTLE_HOME/.install_method"
}
