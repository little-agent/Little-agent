# ============================================================================
# Little Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$LittleHome = "$env:LOCALAPPDATA\little",
    [string]$InstallDir = "$env:LOCALAPPDATA\little\little-agent",
...
$RepoUrlSsh = "git@github.com:little-agent/Little-agent.git"
$RepoUrlHttps = "https://github.com/little-agent/Little-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Little Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Little Agent Team.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $LittleHome)) {
        New-Item -ItemType Directory -Force -Path $LittleHome | Out-Null
    }
    $envFile = Join-Path $LittleHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $LittleHome "node"
...
function Install-Git {
    ...
    which Little uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\little\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Little needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Little copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\little\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``LITTLE_GIT_BASH_PATH`` (User scope) so Little can find it in a fresh
    ...
    # Download PortableGit into $LittleHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $LittleHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Little features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Little needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``LITTLE_GIT_BASH_PATH`` (User env scope) so Little can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $LittleHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $LittleHome\git\usr\bin\bash.exe
    $candidates += "$LittleHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$LittleHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("LITTLE_GIT_BASH_PATH", $candidate, "User")
            $env:LITTLE_GIT_BASH_PATH = $candidate
            Write-Info "Set LITTLE_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Little may not find Git Bash."
    Write-Info "If needed, set LITTLE_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$LittleHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$LittleHome\node;$env:Path"
        Write-Success "Node.js $version found (Little-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $LittleHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $LittleHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\little-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$LittleHome\node") { Remove-Item -Recurse -Force "$LittleHome\node" }
                Move-Item $extractedDir.FullName "$LittleHome\node"
                ...
                $version = & "$LittleHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $LittleHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running little processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/little-agent/Little-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/little-agent/Little-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/little-agent/Little-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\little-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\little-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `little.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install little-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and little.exe broken with
    ...
    # here, not 30 seconds later when the user runs `little`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `little dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `little dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `little dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up little command..."
    
    if ($NoVenv) {
        $littleBin = "$InstallDir"
    } else {
        $littleBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so little is globally available
    # On Windows, the little.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$littleBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$littleBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $littleBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set LITTLE_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\little instead
    # of the Unix default ~/.little
    $currentLittleHome = [Environment]::GetEnvironmentVariable("LITTLE_HOME", "User")
    if (-not $currentLittleHome -or $currentLittleHome -ne $LittleHome) {
        [Environment]::SetEnvironmentVariable("LITTLE_HOME", $LittleHome, "User")
        Write-Success "Set LITTLE_HOME=$LittleHome"
    }
    $env:LITTLE_HOME = $LittleHome
    
    # Update current session
    $env:Path = "$littleBin;$env:Path"
    
    Write-Success "little command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.little directory structure
    New-Item -ItemType Directory -Force -Path "$LittleHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LittleHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$LittleHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.little/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.little/.env"
        }
    } else {
        Write-Info "~/.little/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$LittleHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.little/config.yaml from template"
        }
    } else {
        Write-Info "~/.little/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Little's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.little/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.little/"
    
    # Seed bundled skills into ~/.little/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.little/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.little/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$LittleHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.little/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'little setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\little-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\little-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\little-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.little/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'little setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m little_cli.main setup
    } else {
        python -m little_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $littleCmd = "$InstallDir\venv\Scripts\little.exe"
    if (-not (Test-Path $littleCmd)) {
        $littleCmd = "little"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$LittleHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'little whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: little gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$LittleHome\logs\gateway.log"
            Start-Process -FilePath $littleCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$LittleHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: little gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: little gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$LittleHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$LittleHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$LittleHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$LittleHome\little-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   little              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   little setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   little config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   little config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   little gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   little update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Little repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Little to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/little-agent/Little-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
