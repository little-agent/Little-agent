...
> [little-agent/Little-agent#30136](https://github.com/little-agent/Little-agent/pull/30136)
...
> (`git log --oneline a957ef083..a6f7171a5 -- 'docker/*' 'little_cli/service_manager.py' …`).
> Open Questions are collapsed into a single Decision Log table; full
> deliberations live in PR review comments.

**Goal:** Replace `tini` with s6-overlay as PID 1 in the Little Docker image so
that the main little process, the dashboard, and dynamically-created
...
s6-svscan as PID 1. Main little and the dashboard are declared as static
s6-rc services at image build time. Per-profile gateways — which users create
*after* the image is built (`little profile create coder` →
...
- Existing systemd/launchd/windows surface in `little_cli/gateway.py` and
  `little_cli/gateway_windows.py`.
...
- Interactive TUI must keep working:
  `docker run -it --rm little-agent/Little-agent:latest --tui`.
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  `gateway-default` slot is always registered for the root LITTLE_HOME
  profile so `little gateway start` (no `-p`) has somewhere to land.
...
`ENTRYPOINT [ "/usr/bin/tini", "-g", "--", "/opt/little/docker/entrypoint.sh" ]`.
tini was PID 1, reaped zombies, forwarded SIGTERM to the process group. The
old `docker/entrypoint.sh`:

1. `gosu` privilege drop from root → `little` UID.
2. Copied `.env.example`, `cli-config.yaml.example`, `SOUL.md` into
   `$LITTLE_HOME` if missing.
3. Synced bundled skills via `tools/skills_sync.py`.
4. Optionally backgrounded `little dashboard` in a subshell when
   `LITTLE_DASHBOARD=1` — **not supervised**, no restart.
5. `exec little "$@"` — tini's sole direct child.
...
All init-system logic lives in **`little_cli/gateway.py`** (~5,400 LOC at
re-validation). The systemd/launchd code is ~1,500 lines of that, plus a
separate **`little_cli/gateway_windows.py`** (~690 LOC) for Windows
Scheduled Tasks.
...
- `little_cli/status.py` — gained an `s6` branch for in-container runs.
- `little_cli/profiles.py` — `create_profile` / `delete_profile` register and
  unregister with s6 inside the container (no-op on host).
- `little_cli/doctor.py` — `_check_gateway_service_linger` skips on s6, and a
  new "Service Supervisor" section reports main-little / dashboard /
  profile-gateway counts via the ServiceManager.
- `little_cli/gateway.py::gateway_command` — the
...
`little_cli/container_boot.py`, and auto-starts those whose last recorded
...
- **Root/non-root model:** `/init` runs as root to set up the supervision
  ...
  `s6-setuidgid little` in its `run` script. The per-service `s6-supervise`
  monitor stays root so it can signal its child regardless of UID. Net
  effect: little and all its subprocesses run as UID 10000 exactly as
...
- `/command/with-contenv` sources `/run/s6/container_environment/*` into
  service env — convenient for passing `LITTLE_HOME` etc.
...
- Zombie reaping: PID 1 (s6-svscan) reaps all zombies non-blockingly on
  SIGCHLD. Any subagent subprocess spawned by the main little process is
...
Container ENTRYPOINT is `/init`, PID 1 is s6-svscan. The main little
...
### D2. Main little is an s6 service with container-exit semantics

The contract "container exits when `little` exits" is preserved via a
...
|---|---|
| (no args) | `little` with no args, container exits when little exits |
| `chat -q "..."` | `little chat -q "..."`, container exits with little exit code |
...
- **s6-rc** (declarative, compile-then-swap): used for main little and the
...
(tmpfs, little-writable). s6-svscan picks them up on rescan.
...
(`little -p <profile> gateway run`, or `little gateway run` for the default
...
and silently failed for the unprivileged little user (UID 10000), making
...
Existing per-profile `little -p <profile> gateway start/stop/restart` CLI
...
`little gateway start` (no `-p`) targets a special `gateway-default` slot
that's always registered by the cont-init reconciler. Its run script omits
the `-p` flag and runs against the root `$LITTLE_HOME` profile.

`--all` lifecycle (`little gateway stop --all`, `... restart --all`)
...
main-little service so the little CMD becomes that main program.
...
_is_tui_invocation() {
    ...
    case "${LITTLE_TUI:-}" in 1|true|TRUE|yes) return 0 ;; esac
    if [ -t 0 ] && [ $# -eq 0 ]; then return 0; fi
    return 1
}
```

And in `docker/s6-rc.d/main-little/run`:

```sh
if [ -f /var/run/s6/container_environment/LITTLE_TUI_MODE ]; then
    exec sleep infinity   # s6-overlay will exec CMD as the TTY-connected main
fi
exec s6-setuidgid little little ${LITTLE_ARGS:-}
```

In TUI mode main little is effectively unsupervised (same as the pre-s6
...
| Profile gateway crash loop masks a real config error | Low | Medium | s6 `finish` script `max_restarts` cap (planned follow-up); operators see crash-looping logs in `$LITTLE_HOME/logs/gateways/<profile>/` |
...
| `docker restart` silently loses per-profile gateway registrations (tmpfs scandir wiped) | High (without mitigation) | High | Cont-init reconciliation re-registers from persistent `$LITTLE_HOME/profiles/` and auto-starts those last seen `running`; outcome recorded to `$LITTLE_HOME/logs/container-boot.log` (size-bounded, rotates to `.1` at 256 KiB) |
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `little doctor` alerts when N consecutive container restarts ended in `startup_failed` |
| `_s6_running()` detection works as root but silently fails for unprivileged little user, making runtime-registration path inert | High (without mitigation) | High | **Caught in PR review.** Detection now probes `/proc/1/comm` (world-readable) + `/run/s6/basedir`. Docker integration tests refactored to `docker exec -u little` so the realistic runtime user is exercised |
| `s6-svscanctl` from little hits EACCES on the root-owned control FIFO | Medium | Medium | `02-reconcile-profiles` chowns `/run/service/.s6-svscan/{control,lock}` to little after stage1 creates them |
| Per-service `supervise/control` FIFO is root-owned by s6-supervise, blocking `s6-svc` from little | Known | Medium | Surfaced cleanly as `S6CommandError` (with rc + stderr) instead of raw `CalledProcessError`. Permission fix tracked as a follow-up (small SUID helper, polling chown loop in cont-init.d, or replace `s6-svc` with `down`-marker manipulation) |
...
| # | Question | Decision |
|---|---|---|
| OQ1 | Gate Phase 2 behind env var? | Ship directly (Little is pre-1.0; users can pin the previous image) |
| OQ2 | s6 root model | Root `/init`, drop per-service via `s6-setuidgid little` |
| OQ3 | Dashboard opt-in mechanism | Always declared as an s6 service; `03-dashboard-toggle` cont-init script writes a `down` marker when `LITTLE_DASHBOARD` is unset so `s6-svstat` reports the slot's real state |
| OQ4 | Podman rootless | Supported, fix reactively |
| OQ5 | Service naming | `gateway-<profile>` (matches pre-existing `little-gateway-<profile>.service` systemd convention) |
...
| OQ8 | Log persistence | `$LITTLE_HOME/logs/gateways/<profile>/`. The log path is sourced from runtime `$LITTLE_HOME` via `with-contenv`, NOT Python-substituted at registration time |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `little gateway start` (no `-p`) has somewhere to land.
...
- [x] `docker run -it --rm little-agent --tui` starts the Ink TUI with
      working keyboard input, cursor control, and resize (SIGWINCH)
- [x] Dashboard crashes are recovered by s6 within ~2s
- [x] `little profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `little -p test gateway start` inside a container dispatches through s6
- [x] `little -p test gateway stop` inside a container cleanly stops via s6
- [x] `little profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$LITTLE_HOME/logs/gateways/test/current`
- [x] `little status` inside the container shows `Manager: s6`
- [x] `little gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `little gateway stop --all` / `... restart --all` iterate every
...
- [x] `little gateway install/start/stop` on Linux host and macOS host
