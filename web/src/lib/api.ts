...
// (e.g. https://mission-control.tilos.com/little/). The Python backend
// injects ``window.__LITTLE_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__LITTLE_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const LITTLE_BASE_PATH = readBasePath();
const BASE = LITTLE_BASE_PATH;
...
declare global {
  interface Window {
    __LITTLE_SESSION_TOKEN__?: string;
    __LITTLE_BASE_PATH__?: string;
    ...
    __LITTLE_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Little-Session-Token";
...
export async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    // (``little update``, ``little gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__LITTLE_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__LITTLE_AUTH_REQUIRED__) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("little.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("little.tokenReloadAttempted", "1");
        } catch {
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__LITTLE_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Little dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``little_cli.web_server._ws_auth_ok``)
...
export const api = {
  ...
  updateLittle: () =>
    fetchJSON<ActionResponse>("/api/little/update", { method: "POST" }),
