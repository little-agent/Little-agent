...
This module provides the glue:
    - ``LittleTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Little Agent"
"""
...
from little_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__little_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses LITTLE_HOME so each profile gets its own OAuth tokens.
    Layout: ``LITTLE_HOME/mcp-tokens/``
    """
    try:
        from little_constants import get_little_home
        base = Path(get_little_home())
    except ImportError:
        base = Path(os.environ.get("LITTLE_HOME", str(Path.home() / ".little")))
...
# ---------------------------------------------------------------------------
# LittleTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class LittleTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        LITTLE_HOME/mcp-tokens/<server_name>.json         -- tokens
        LITTLE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        LITTLE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Little records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Little.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://little-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `little mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = LittleTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Little Agent")
    scope = cfg.get("scope")
