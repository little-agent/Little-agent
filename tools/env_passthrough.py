...
def _is_little_provider_credential(name: str) -> bool:
    """True if ``name`` is a Little-managed provider credential (API key,
    token, or similar) per ``_LITTLE_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Little API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _LITTLE_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _LITTLE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Little-managed provider credentials (from
    ``_LITTLE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Little-managed
    ...
    Non-Little third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_little_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Little provider "
                "credential %r (blocked by _LITTLE_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Little-managed provider credentials must not be passed
                ...
                if _is_little_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Little "
                        "provider credential %r from config.yaml (blocked "
                        "by _LITTLE_PROVIDER_ENV_BLOCKLIST). Operator "
