...
Backend is selected during ``little tools`` setup (web.backend in config.yaml).
When available, Little can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.little/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``little tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `little tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "little-agent.com" or host.endswith(".little-agent.com")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `little tools` to set one up."
                ),
            }
        else:
            logger.info(
