#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Little Skills Hub.
...
Used by little_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            little_meta = metadata.get("little", {})
            if isinstance(little_meta, dict):
                tags = little_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                little_meta = meta_block.get("little", {})
                if isinstance(little_meta, dict):
                    tags = little_meta.get("tags", [])
...
def _load_little_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for LITTLE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if LITTLE_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - LITTLE_INDEX_CACHE_FILE.stat().st_mtime
            if age < LITTLE_INDEX_TTL:
                return json.loads(LITTLE_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(LITTLE_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Little index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Little index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        LITTLE_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        LITTLE_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class LittleIndexSource(SkillSource):
    """Skill source backed by the centralized Little Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_little_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "little-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "little-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        LittleIndexSource(auth=auth), # Centralized index (search + resolved install paths)
