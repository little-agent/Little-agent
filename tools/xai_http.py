...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``little tools`` repaint, tool-registration scans,
    ...
    2. ``~/.little/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_little_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.little/.env`` first, then ``os.environ``.

    Wraps :func:`little_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from little_cli.config import get_env_value as _little_get_env_value

        value = _little_get_env_value(name)
...
def little_xai_user_agent() -> str:
    """Return a stable Little-specific User-Agent for xAI HTTP calls."""
    try:
        from little_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Little-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Little-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``little_cli.config.get_env_value`` so keys
    stored in ``~/.little/.env`` (the standard Little location) are honored —
