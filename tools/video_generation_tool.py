...
provider** — turn on a backend by enabling a plugin (``little plugins
enable video_gen/<name>``) and selecting it in ``little tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/little-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `little tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `little plugins list` to see "
            f"installed video gen backends, or `little tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `little tools` → "
...
# `little tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`little tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `little tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
