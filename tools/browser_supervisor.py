...
One ``CDPSupervisor`` runs per Little ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "little-dialog-bridge.invalid"
...
(() => {
  if (window.__littleDialogBridgeInstalled) return;
  window.__littleDialogBridgeInstalled = true;
  const ENDPOINT = "http://little-dialog-bridge.invalid/";
