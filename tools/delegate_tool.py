...
# Must match little_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (little-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("little-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``little-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. little-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.little/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'little auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (little_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Little subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
