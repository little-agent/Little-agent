...
it is automatically downloaded from GitHub releases to $LITTLE_HOME/bin/tirith.
...
def _get_little_home() -> str:
    """Return the Little home directory, respecting LITTLE_HOME env var."""
    return str(get_little_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_little_home(), ".tirith-install-failed")
...
def _little_bin_dir() -> str:
    """Return $LITTLE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_little_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LITTLE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_little_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $LITTLE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $LITTLE_HOME/bin/tirith
    ...
    little_bin = os.path.join(_little_bin_dir(), "tirith")
    if os.path.isfile(little_bin) and os.access(little_bin, os.X_OK):
        _resolved_path = little_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return little_bin
