...
Copies bundled skills from the repo's skills/ directory into ~/.little/skills/
...
The manifest lives at ~/.little/skills/.bundled_manifest.
"""
...
from little_constants import get_bundled_skills_dir, get_little_home, get_optional_skills_dir
...
def _get_bundled_dir() -> Path:
    ...
    Checks LITTLE_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.little/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.little/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `little skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `little skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `little update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.little/skills/ ...")
