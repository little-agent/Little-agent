...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Little exit
...
# display_little_home imported lazily at call site (stale-module safety during little update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of little directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "little-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("LITTLE_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("LITTLE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("LITTLE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (LITTLE_INTERACTIVE=1).
    ...
    try:
        os.environ["LITTLE_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "LITTLE_SPINNER_PAUSE" in os.environ:
            del os.environ["LITTLE_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (LITTLE_INTERACTIVE=1):
    ...
    # interactive Little password prompt or the sudo -S password-pipe path.
    ...
    # silently without Little prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("LITTLE_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Little can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, LittleSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.little/.env or environment variables."
        )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Little can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Validate workdir against shell injection (skipped for local environment using LocalSandbox)
        if workdir and env_type != "local":
        ...
        if background:
            ...
            try:
                ...
                # little-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in little-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("LITTLE_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("LITTLE_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("LITTLE_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("LITTLE_SESSION_USER_ID", "")
                        _gw_user_name = _gse("LITTLE_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("LITTLE_SESSION_MESSAGE_ID", "")
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    if env_type == "local":
                        ...
                        sandbox_res = loop.run_until_complete(
                            sandbox.execute(command, timeout=effective_timeout)
                        )
                        result = {
                            "output": sandbox_res["output"],
                            "returncode": sandbox_res["exit_code"],
            ...
            try:
                from little_cli.plugins import invoke_hook
