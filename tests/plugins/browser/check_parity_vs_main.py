...
Run from the PR worktree:
    ...
    cd ~/.little/little-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.little/little-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.little/little-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a little-agent checkout"
)
...
# Isolated LITTLE_HOME for the config write.
