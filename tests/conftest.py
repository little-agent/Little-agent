"""Shared fixtures for the little-agent test suite.
...
2. **Isolated LITTLE_HOME.** LITTLE_HOME points to a per-test tempdir so
   code reading ``~/.little/*`` via ``get_little_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".little"`` instead of the canonical
   ``get_little_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No LITTLE_SESSION_* inheritance** — the agent's current gateway
...
# LITTLE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_LITTLE_BEHAVIORAL_VARS = frozenset({
    "LITTLE_YOLO_MODE",
    "LITTLE_INTERACTIVE",
    "LITTLE_QUIET",
    ...
    "LITTLE_SESSION_CHAT_ID",
    "LITTLE_SESSION_CHAT_NAME",
    "LITTLE_SESSION_THREAD_ID",
    "LITTLE_SESSION_SOURCE",
    "LITTLE_SESSION_KEY",
    "LITTLE_GATEWAY_SESSION",
    "LITTLE_PLATFORM",
    "LITTLE_MODEL",
    "LITTLE_INFERENCE_MODEL",
    "LITTLE_INFERENCE_PROVIDER",
    "LITTLE_TUI_PROVIDER",
    "LITTLE_MANAGED",
    "LITTLE_DEV",
    "LITTLE_CONTAINER",
    ...
    "LITTLE_TIMEZONE",
    "LITTLE_REDACT_SECRETS",
    "LITTLE_BACKGROUND_NOTIFICATIONS",
    "LITTLE_EXEC_ASK",
    "LITTLE_HOME_MODE",
    "LITTLE_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.little/kanban.db instead of the per-test LITTLE_HOME.
    "LITTLE_KANBAN_DB",
    "LITTLE_KANBAN_BOARD",
    "LITTLE_KANBAN_HOME",
    "LITTLE_KANBAN_WORKSPACES_ROOT",
    "LITTLE_KANBAN_LOGS_ROOT",
    "LITTLE_KANBAN_TASK",
    "LITTLE_KANBAN_WORKSPACE",
    "LITTLE_KANBAN_RUN_ID",
    "LITTLE_KANBAN_CLAIM_LOCK",
    "LITTLE_KANBAN_DISPATCH_IN_GATEWAY",
    "LITTLE_TENANT",
    ...
    "LITTLE_DASHBOARD_OAUTH_CLIENT_ID",
    "LITTLE_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LITTLE_HOME to per-test tempdirs so code that
    reads ``~/.little/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral LITTLE_* vars that could change test semantics.
    for name in _LITTLE_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect LITTLE_HOME to a per-test tempdir. Code that reads
    #    ``~/.little/*`` via ``get_little_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.little/*`` via
    #    ``Path.home() / ".little"`` instead of ``get_little_home()``
    #    is a bug to fix at the callsite.
    fake_little_home = tmp_path / "little_test"
    fake_little_home.mkdir()
    (fake_little_home / "sessions").mkdir()
    (fake_little_home / "cron").mkdir()
    (fake_little_home / "memories").mkdir()
    (fake_little_home / "skills").mkdir()
    monkeypatch.setenv("LITTLE_HOME", str(fake_little_home))
    ...
    #    ~/.little/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import little_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_little_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal little config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``little-gateway`` process
...
# correlated with a ``tests/little_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> little-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart little-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting little/python patterns are also blocked.
    """
    ...
    def _matches_little_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _LITTLE_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_little_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch little-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m little_cli.main".
                if (
                    "little" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live little-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting little/python could hit the live gateway. "
