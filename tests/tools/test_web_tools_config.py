...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "little-agent.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.little-agent.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "little-agent.com",
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.little-agent.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_little_home_override(self, tmp_path):
        """Auth lookup should read from LITTLE_HOME/auth.json, not ~/.little/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".little").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``little tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
