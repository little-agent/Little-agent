...
def test_little_provider_subclass_exists():
    """LittleMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _LITTLE_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _LITTLE_PROVIDER_CLS is not None
    assert issubclass(_LITTLE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_little_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns LittleMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _LITTLE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("LITTLE_HOME", str(tmp_path))
    ...
    assert _LITTLE_PROVIDER_CLS is not None
    assert isinstance(provider, _LITTLE_PROVIDER_CLS)
    assert provider._little_server_name == "srv"
