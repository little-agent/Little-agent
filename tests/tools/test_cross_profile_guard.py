...
def fake_little(tmp_path, monkeypatch):
    """Build a two-profile Little layout and point LITTLE_HOME at
    the little-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_little):
        from tools.file_tools import write_file_tool
        target = fake_little["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_little):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_little["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "little-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_little):
    ...
    def test_non_little_path_unaffected(self, fake_little, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_little):
    ...
    def test_cross_profile_patch_bypass(self, fake_little):
        from tools.file_tools import patch_tool
        target = fake_little["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_little):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_little, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_little["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up LITTLE_HOME (set in
        ...
        assert "not found in active profile 'little-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_little, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_little["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_little["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.little/profiles/<name>/."""
        # Don't set LITTLE_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_little_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_little_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_little):
        """When active profile is 'little-security', the prompt warns
        ...
        assert "Active Little profile" in src
        assert "cross_profile=True" in src
        assert "~/.little/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Little profile: default" in src
        assert "Active Little profile: {active_profile}" in src
