...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._little_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(little_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(little_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_little_bin(self):
        """After _INSTALL_FAILED, manual install in LITTLE_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        little_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(little_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(little_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._little_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == little_bin
            assert _tirith_mod._resolved_path == little_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# LITTLE_HOME isolation
# ---------------------------------------------------------------------------

class TestLittleHomeIsolation:
    def test_little_bin_dir_respects_little_home(self):
        """_little_bin_dir must use LITTLE_HOME, not hardcoded ~/.little."""
        from tools.tirith_security import _little_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"LITTLE_HOME": tmpdir}):
            result = _little_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_little_home(self):
        """_failure_marker_path must use LITTLE_HOME, not hardcoded ~/.little."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LITTLE_HOME": "/custom/little"}):
            result = _failure_marker_path()
        assert result == "/custom/little/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets LITTLE_HOME; verify it's active."""
        little_home = os.getenv("LITTLE_HOME")
        assert little_home is not None, "LITTLE_HOME should be set by conftest"
        assert "little_test" in little_home, "Should point to test temp dir"

    def test_get_little_home_fallback(self):
        """Without LITTLE_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_little_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove LITTLE_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".little")
            result = _get_little_home()
