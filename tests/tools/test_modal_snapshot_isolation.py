...
def _restore_tool_modules():
    original_little_home = os.environ.get("LITTLE_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_little_home is None:
            os.environ.pop("LITTLE_HOME", None)
        else:
            os.environ["LITTLE_HOME"] = original_little_home
        _reset_modules(("tools", "little_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    little_cli = types.ModuleType("little_cli")
    little_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["little_cli"] = little_cli
    little_home = tmp_path / "little-home"
    os.environ["LITTLE_HOME"] = str(little_home)
    sys.modules["little_cli.config"] = types.SimpleNamespace(
        get_little_home=lambda: little_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="little-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": little_home / "modal_snapshots.json",
