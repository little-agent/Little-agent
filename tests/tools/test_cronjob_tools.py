...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `little-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.little/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.little/.env to steal credentials"
        ) == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because little uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("LITTLE_GATEWAY_SESSION", "1")
        monkeypatch.delenv("LITTLE_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("LITTLE_INTERACTIVE", raising=False)
        monkeypatch.delenv("LITTLE_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("LITTLE_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("LITTLE_INTERACTIVE", false_like_value)
        monkeypatch.delenv("LITTLE_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("LITTLE_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["LITTLE_INTERACTIVE", "LITTLE_GATEWAY_SESSION", "LITTLE_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("LITTLE_INTERACTIVE", "LITTLE_GATEWAY_SESSION", "LITTLE_EXEC_ASK"):
