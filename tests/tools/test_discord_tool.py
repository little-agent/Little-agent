...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in little-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_little_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["little-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["little-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _LITTLE_CORE_TOOLS
        assert "discord" not in _LITTLE_CORE_TOOLS
        assert "discord_admin" not in _LITTLE_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"little-discord", "little-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("little_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["little-discord"], quiet_mode=True)
