...
class TestWriteDenyExactPaths:
    ...
    def test_little_env(self):
        # ``.env`` under the active LITTLE_HOME (profile-aware, not just
        # ``~/.little``) must be write-denied. The hermetic test conftest
        # points LITTLE_HOME at a tempdir — resolve via get_little_home()
        # to match the denylist.
        from little_constants import get_little_home
        path = str(get_little_home() / ".env")
        assert _is_write_denied(path) is True

    def test_little_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.little/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "little_root"
        ...
        # Sanity check: LITTLE_HOME does point to the profile dir, not the root.
        from little_constants import get_little_home, get_default_little_root
        assert get_little_home() == profile_home
        assert get_default_little_root() == root
...
class TestWriteAllowed:
    ...
    def test_little_config_not_env(self):
        path = os.path.join(str(Path.home()), ".little", "config.yaml")
