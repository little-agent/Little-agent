...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "LITTLE_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "LITTLE_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
            ...
            get_session_env_mock.side_effect = lambda name, default="": {
                ...
                "LITTLE_SESSION_USER_ID": "user-123",
            }.get(name, default)
            result = json.loads(
                send_message_tool(
                    {
    ...
    def test_media_tag_outside_allowed_roots_is_not_sent(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LITTLE_MEDIA_TRUST_RECENT_FILES", "0")
        config, telegram_cfg = _make_config()
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Little](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Little>",
        )
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from little")
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@little:matrix.org")
        assert chat_id == "@little:matrix.org"
        assert thread_id is None
        assert is_explicit is True
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``little cron``
    runs separately from ``little gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``LITTLE_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``LITTLE_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``LITTLE_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring LITTLE_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a LITTLE_HOME that has no
        ...
        with patch("gateway.session_context.get_session_env",
                   side_effect=AssertionError("session_context not consulted "
                                              "when LITTLE_KANBAN_TASK is set")), \
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``LITTLE_SESSION_PLATFORM=local`` means CLI-style — must defer to
