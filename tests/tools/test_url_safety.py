...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        monkeypatch.delenv("LITTLE_ALLOW_PRIVATE_URLS", raising=False)
        with patch("little_cli.config.read_raw_config", side_effect=Exception("no config")):
            assert _global_allow_private_urls() is False

    def test_env_var_true(self, monkeypatch):
        """LITTLE_ALLOW_PRIVATE_URLS=true enables the toggle."""
        monkeypatch.setenv("LITTLE_ALLOW_PRIVATE_URLS", "true")
        assert _global_allow_private_urls() is True

    def test_env_var_1(self, monkeypatch):
        """LITTLE_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("LITTLE_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """LITTLE_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("LITTLE_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """LITTLE_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("LITTLE_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
    ...
    def test_config_security_section(self, monkeypatch):
        ...
        with patch("little_cli.config.read_raw_config", return_value=cfg):
            assert _global_allow_private_urls() is True
...
class TestAllowPrivateUrlsIntegration:
    ...
    def test_benchmark_ip_allowed_when_toggle_on(self, monkeypatch):
        ...
        with patch("socket.getaddrinfo", return_value=[
            ...
            assert is_safe_url("https://little-agent.com") is True
