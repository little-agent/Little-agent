...
def _clean_state():
    ...
    saved = {}
    for k in ("LITTLE_INTERACTIVE", "LITTLE_GATEWAY_SESSION", "LITTLE_EXEC_ASK", "LITTLE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LITTLE_INTERACTIVE"] = "1"
        session_key = os.getenv("LITTLE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LITTLE_INTERACTIVE or LITTLE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
