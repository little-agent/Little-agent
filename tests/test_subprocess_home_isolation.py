...
See: https://github.com/little-agent/Little-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for little_constants.get_subprocess_home()."""

    def test_returns_none_when_little_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LITTLE_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("LITTLE_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from little_constants import (
            get_little_home,
            reset_little_home_override,
            set_little_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_little_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_little_home_override(profile)
        try:
            assert get_little_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_little_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_little_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(little_home / "home")
    ...
    def test_no_injection_when_little_home_unset(self, monkeypatch):
        monkeypatch.delenv("LITTLE_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["LITTLE_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from little_cli.profiles import _PROFILE_DIRS
