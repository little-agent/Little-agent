...
from little_cli.profiles import _get_default_little_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "little"
        assert config.workspace_id == "little"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.little/honcho.json
        monkeypatch.setenv("LITTLE_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"little": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"little": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"little": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="little-agent"
        ):
            result = config.resolve_session_name("/home/user/little-agent/subdir")
        assert result == "little-agent"
...
class TestResolveConfigPath:
    def test_prefers_little_home_when_exists(self, tmp_path):
        little_home = tmp_path / "little"
        little_home.mkdir()
        local_cfg = little_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"LITTLE_HOME": str(little_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: LITTLE_HOME points at ~/.little/profiles/<name>, so
        # _get_default_little_home() must resolve back to ~/.little — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".little"
        ...
        assert _get_default_little_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_little_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("LITTLE_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".little".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".little"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_little(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("LITTLE_HONCHO_HOST", None)
            os.environ.pop("LITTLE_HOME", None)
            assert resolve_active_host() == "little"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"LITTLE_HONCHO_HOST": "little.coder"}):
            assert resolve_active_host() == "little.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_HONCHO_HOST", None)
            with patch("little_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "little.coder"

    def test_default_profile_returns_little(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_HONCHO_HOST", None)
            with patch("little_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "little"

    def test_custom_profile_returns_little(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_HONCHO_HOST", None)
            with patch("little_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "little"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_HONCHO_HOST", None)
            # Temporarily remove little_cli.profiles to simulate import failure
            saved = sys.modules.get("little_cli.profiles")
            sys.modules["little_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "little"
            finally:
                if saved is not None:
                    sys.modules["little_cli.profiles"] = saved
                else:
                    sys.modules.pop("little_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="little.coder")
        assert config.host == "little.coder"
        assert config.workspace_id == "little"  # shared workspace
        assert config.ai_peer == "little.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="little")
        assert config.host == "little"
        assert config.workspace_id == "little"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "little": {"aiPeer": "little", "peerName": "alice"},
                "little.coder": {
                    "aiPeer": "little.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="little.coder", config_path=config_file,
        )
        assert config.host == "little.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "little.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="little.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "little.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"little": {"enabled": True, "aiPeer": "little"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"little": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_little_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="little",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("little_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("little_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_little_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="little",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("little_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
