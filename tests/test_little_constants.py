"""Tests for little_constants module."""
...
class TestGetDefaultLittleRoot:
    """Tests for get_default_little_root() — Docker/custom deployment awareness."""

    def test_no_little_home_returns_native(self, tmp_path, monkeypatch):
        """When LITTLE_HOME is not set, returns ~/.little."""
        monkeypatch.delenv("LITTLE_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_little_root() == tmp_path / ".little"

    def test_little_home_is_native(self, tmp_path, monkeypatch):
        """When LITTLE_HOME = ~/.little, returns ~/.little."""
        native = tmp_path / ".little"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(native))
        assert get_default_little_root() == native

    def test_little_home_is_profile(self, tmp_path, monkeypatch):
        """When LITTLE_HOME is a profile under ~/.little, returns ~/.little."""
        native = tmp_path / ".little"
    ...
    def test_little_home_is_docker(self, tmp_path, monkeypatch):
        """When LITTLE_HOME points outside ~/.little (Docker), returns LITTLE_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(docker_home))
        assert get_default_little_root() == docker_home

    def test_little_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LITTLE_HOME outside ~/.little is treated as the root."""
        custom = tmp_path / "my-little-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(custom))
        assert get_default_little_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (LITTLE_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(profile))
        assert get_default_little_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(little_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(little_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".little"
