...
``little_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``little --help``, ``little version``,
``little logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``little --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from little_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["little", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["little", "-w", "--tui"], None),
        (["little", "version"], "version"),
        (["little", "--tui", "chat"], "chat"),
        (["little", "-w", "logs"], "logs"),
        (["little", "chat", "hello world"], "chat"),
        (["little", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["little", "-m", "gpt5", "chat"], "chat"),
        (["little", "--model", "gpt5", "chat", "hi"], "chat"),
        (["little", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["little", "-z", "hello world"], None),
        (["little", "-z", "hello", "chat"], "chat"),
        (["little", "--model=gpt5", "chat"], "chat"),     # inline form
        (["little", "--", "chat"], "chat"),               # -- terminator
        (["little", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["little", "some-plugin-cmd"], "some-plugin-cmd"),
        (["little", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["little"],                          # bare → chat
        ["little", "--help"],                # top-level help
        ["little", "-h"],
        ["little", "version"],               # known built-in
        ["little", "logs"],
        ["little", "gateway", "run"],
        ["little", "--tui"],
        ["little", "-w", "--tui"],
        ["little", "chat", "hi"],
        ["little", "help"],                  # accepted built-in-ish
        ["little", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["little", "meet", "join"],          # potential google_meet plugin
        ["little", "honcho", "status"],      # potential memory plugin
        ["little", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``little help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"little_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
