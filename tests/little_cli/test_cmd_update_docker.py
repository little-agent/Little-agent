"""Tests for ``little update`` / ``--check`` inside the Docker container.
...
fix, ``little update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Little, not an update to the running container, so the message
...
from little_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("little_cli.config.is_managed", return_value=False)
@patch("little_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``little update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull little-agent/Little-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``little update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull little-agent/Little-agent:latest" in msg
    ...
    assert "LITTLE_HOME" in msg or "/opt/data" in msg, (
