...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LITTLE_HOME with .container-mode file."""
    little_home = tmp_path / ".little"
    little_home.mkdir()
    monkeypatch.setenv("LITTLE_HOME", str(little_home))
    monkeypatch.delenv("LITTLE_DEV", raising=False)

    container_mode = little_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=little-agent\n"
        "exec_user=little\n"
        "little_bin=/data/current-package/bin/little\n"
    )
    return little_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("little_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "little-agent"
    assert info["exec_user"] == "little"
    assert info["little_bin"] == "/data/current-package/bin/little"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("little_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_little_dev(container_env, monkeypatch):
    """Returns None when LITTLE_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LITTLE_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_little_dev_zero(container_env, monkeypatch):
    """LITTLE_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LITTLE_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        little_home = Path(tmpdir) / ".little"
        little_home.mkdir()
        (little_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("little_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_little_home": lambda: little_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=little-custom\n"
        "exec_user=myuser\n"
        "little_bin=/opt/little/bin/little\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "little-custom"
    assert info["exec_user"] == "myuser"
    assert info["little_bin"] == "/opt/little/bin/little"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from little_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "little"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "little-agent" in cmd
    assert "/data/current-package/bin/little" in cmd
    assert "chat" in cmd
