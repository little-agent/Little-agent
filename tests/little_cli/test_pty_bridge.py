"""Unit tests for little_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf little-ok"])
        try:
            output = _read_until(bridge, b"little-ok")
            assert b"little-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$LITTLE_PTY_TEST\""],
            env={**os.environ, "LITTLE_PTY_TEST": "pty-env-works"},
        )
        try:
