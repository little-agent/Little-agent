"""Tests for SIGHUP protection and stdout mirroring in ``little update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``little_cli/main.py``.  These exist so
that ``little update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_little_home if present
        import little_cli.config as _cfg
        if hasattr(_cfg, "_LITTLE_HOME_CACHE"):
            _cfg._LITTLE_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "little update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_little_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "little_cli.config.get_little_home", _boom, raising=True
        )
