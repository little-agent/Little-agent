"""Tests for IRC gateway configuration via `little setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Little install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Little install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#little")
            monkeypatch.setenv("IRC_NICKNAME", "little-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `little setup gateway` experience with IRC present."""
