...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.little-agent.com/v1"
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="little_cli.auth"):
            assert (
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.little-agent.com is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.little-agent.com would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.little-agent.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="little_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.little-agent.com/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.little-agent.com``, they MUST update the allowlist
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        import little_cli.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import little_cli.proxy.adapters.nous_portal as _nous_adapter
