...
When ``LITTLE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.little/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.little            (has its own auth.json fixture)
    * Profile     -> tmp_path/.little/profiles/coder   (active, LITTLE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Little is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from little_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    little_home = tmp_path / "classic"
    little_home.mkdir()
    monkeypatch.setenv("LITTLE_HOME", str(little_home))

    _write(little_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (LITTLE_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.little as the "real home" — which is NOT equal
    # to the LITTLE_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    little_home = tmp_path / "classic"
    ...
    _write(little_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from little_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: LITTLE_HOME is set to a custom path that is NOT under
    # ~/.little/profiles/ — get_default_little_root() returns LITTLE_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from little_cli.auth import read_credential_pool, write_credential_pool
