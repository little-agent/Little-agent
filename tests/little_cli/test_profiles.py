"""Comprehensive tests for little_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and LITTLE_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.little/profiles)
    * LITTLE_HOME  -> tmp_path/.little  (so get_little_home() agrees)
    * Creates the bare-minimum ~/.little directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["little", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Little install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_little_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".little" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".little" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.little must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".little"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes little-agent, .worktrees,
        ...
        (default_home / "little-agent" / ".git").mkdir(parents=True)
        (default_home / "little-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "little-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "little-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `little profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `little update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("little_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".little" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_little_home_returns_default(self, profile_env):
        # LITTLE_HOME points to tmp_path/.little which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom LITTLE_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("LITTLE_HOME", str(custom))
        # With Docker-aware roots, a custom LITTLE_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("little")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "little.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["little.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["little.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "little.ssi_health": {"workspace": "little", "enabled": True}
            }
        }))
        ...
        assert "little.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["little.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["little.heimdall"]["workspace"] == "little"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "little.ssi_health": {"aiPeer": "ssi_health"},
                "little.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["little.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["little.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("little-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".little_history",
        ...
        excluded_prefixes = [
            "default/little-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.little_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultLittleHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_little_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".little" / "profiles"

    def test_default_little_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_little_home()
        assert home == tmp_path / ".little"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (LITTLE_HOME outside ~/.little), profiles go under LITTLE_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_little_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_little_home() returns LITTLE_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (LITTLE_HOME under ~/.little), profiles root is still ~/.little/profiles."""
        native = tmp_path / ".little"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under LITTLE_HOME."""
        from little_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under LITTLE_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LITTLE_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".little" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from little_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".little"
