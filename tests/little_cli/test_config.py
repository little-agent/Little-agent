"""Tests for little_cli configuration management."""
...
class TestGetLittleHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LITTLE_HOME", None)
            home = get_little_home()
            assert home == Path.home() / ".little"

    def test_env_override(self):
        with patch.dict(os.environ, {"LITTLE_HOME": "/custom/path"}):
            home = get_little_home()
            assert home == Path("/custom/path")


class TestEnsureLittleHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``little logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from little_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="little_cli.config"):
                config = load_config()
            ...
            # stands out at little startup before logging is configured)
            captured = capsys.readouterr()
            assert "little config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path)}):
            ...
            assert "little config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LITTLE_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from little_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``LITTLE_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Little
    process on next startup via the dotenv → ``os.environ`` chain in
    ``little_cli/env_loader.py``.
    ...
    def _little_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "LITTLE_SPOTIFY_CLIENT_ID",
            "LITTLE_QWEN_BASE_URL",
            "LITTLE_MAX_ITERATIONS",
        ],
    )
    def test_little_integration_keys_still_writable(self, allowed_key):
        """``LITTLE_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``LITTLE_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``LITTLE_*``) keep working. The denylist
