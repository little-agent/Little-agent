"""Tests for the `little memory reset` CLI command.
...
- Profile-scoped reset (uses LITTLE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LITTLE_HOME with memory files."""
    little_home = tmp_path / ".little"
    memories = little_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("LITTLE_HOME", str(little_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nLittle repo is at ~/.little/little-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `little memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        little_home = tmp_path / ".little"
        (little_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("LITTLE_HOME", str(little_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LITTLE_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        little_home = tmp_path / ".little"
        little_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("LITTLE_HOME", str(little_home))

        # The memories dir won't exist; get_little_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
