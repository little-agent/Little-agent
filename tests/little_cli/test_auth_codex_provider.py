"""Tests for Codex auth — tokens stored in Little auth store (~/.little/auth.json)."""
...
def _setup_little_auth(little_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Little auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_little_auth(little_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_little_auth(little_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("LITTLE_HOME", str(little_home))
    ...
    monkeypatch.setattr("little_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    little_home = tmp_path / "little"
    _setup_little_auth(little_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("LITTLE_HOME", str(little_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((little_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``little auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Little auth store, not ~/.codex/."""
    little_home = tmp_path / "little"
    codex_home = tmp_path / "codex-cli"
    little_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "little-at", "refresh_token": "little-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Little store
    assert not (codex_home / "auth.json").exists()

    # Little auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "little-at"


def test_resolve_returns_little_auth_store_source(tmp_path, monkeypatch):
    little_home = tmp_path / "little"
    _setup_little_auth(little_home)
    monkeypatch.setenv("LITTLE_HOME", str(little_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run little auth".
    """
    from little_cli.auth import (
    ...
    assert "little auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from little_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from little_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "little_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("little_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("little_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
