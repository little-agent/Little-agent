...
Covers:
 - Normal path (tokens already in Little auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Little
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `little auth openai-codex`. The old
...
@pytest.fixture()
def little_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Little auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(little_auth_only_env):
    """openai-codex appears when tokens are already in Little auth store."""
...
def test_codex_picker_uses_live_codex_catalog(little_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Little
    auth store."""
    little_home = tmp_path / ".little"
    little_home.mkdir()
