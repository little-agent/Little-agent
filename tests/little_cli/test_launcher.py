"""Tests for the top-level `./little` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./little` should use `little_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "little"
    called = []

    fake_main_module = types.ModuleType("little_cli.main")

    def fake_main():
        called.append("little_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "little_cli.main", fake_main_module)
