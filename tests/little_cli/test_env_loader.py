...
def test_user_env_overrides_stale_shell_values(tmp_path, monkeypatch):
    ...
    loaded = load_little_dotenv(little_home=home)
...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_little_dotenv(little_home=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nLITTLE_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("little_cli.main", None)
    importlib.import_module("little_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("LITTLE_INFERENCE_PROVIDER") == "custom"
