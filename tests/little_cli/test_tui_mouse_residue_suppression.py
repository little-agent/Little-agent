...
The Python launcher (`little --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Little session
...
from little_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["little", "--tui", "-c", "abc"])
        monkeypatch.delenv("LITTLE_TUI", raising=False)
        monkeypatch.delenv("LITTLE_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_little_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["little"])
        monkeypatch.setenv("LITTLE_TUI", "1")
        monkeypatch.delenv("LITTLE_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("LITTLE_TUI", raising=False)
        monkeypatch.setenv("LITTLE_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `little --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
