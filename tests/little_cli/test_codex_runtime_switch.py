...
class TestApply:
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate Little mcp_servers
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("little_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "little-tools"]
        ...
        # User MCP servers are reported (excluding internal little-tools)
        ...
        # Little tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("little_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
