"""Tests for the ``little send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``little_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from little_cli import send_cmd
...
def test_load_little_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``little send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``little config set``) lives
    ...
    little_home.mkdir()
    (little_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_little_home() to re-resolve under the patched env.
    from importlib import reload

    import little_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_little_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_little_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    little_home = tmp_path / ".little"
    little_home.mkdir()
    (little_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_little_env_handles_missing_files(tmp_path, monkeypatch):
