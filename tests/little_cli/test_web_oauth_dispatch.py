"""Regression tests for the OAuth dispatcher in little_cli.web_server.
...
from little_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Little-Session-Token": _SESSION_TOKEN}
...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.little-agent.com/device",
        "verification_uri_complete": (
            "https://portal.little-agent.com/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.little-agent.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "little_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "little_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "little_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_honors_legacy_scope_override(monkeypatch):
    ...
    from little_cli import web_server as ws
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "little_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "little_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "little auth add hypothetical-pkce-provider",
