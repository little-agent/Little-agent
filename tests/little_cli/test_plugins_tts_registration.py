...
``$LITTLE_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/little_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        from little_cli.plugins import PluginManager

        from agent import tts_registry
        tts_registry._reset_for_tests()

        little_home = Path(os.environ["LITTLE_HOME"])
        _write_plugin(
            little_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(little_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(little_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(little_home, "shadow-tts-plugin")
