...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (LITTLE_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from little_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via LITTLE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in LITTLE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from little_cli.providers import LITTLE_OVERLAYS
        assert "bedrock" in LITTLE_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from little_cli.providers import LITTLE_OVERLAYS
        assert LITTLE_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from little_cli.providers import LITTLE_OVERLAYS
        assert LITTLE_OVERLAYS["bedrock"].auth_type == "aws_sdk"
