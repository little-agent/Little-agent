...
binary download are exercised manually by `little secrets bitwarden
...
def little_home(tmp_path, monkeypatch):
    """Point Little at an isolated home directory."""
    home = tmp_path / ".little"
    home.mkdir()
    monkeypatch.setenv("LITTLE_HOME", str(home))
    # Some modules cache get_little_home; clear if needed.
    import little_constants
    if hasattr(little_constants, "_LITTLE_HOME_CACHE"):
        little_constants._LITTLE_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(little_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(little_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(little_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from little_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under little_home/cache/."""
    home = tmp_path / ".little"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
