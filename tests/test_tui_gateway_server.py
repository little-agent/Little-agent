...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["LITTLE_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("LITTLE_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("LITTLE_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("LITTLE_VOICE", "1")
    monkeypatch.delenv("LITTLE_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "little_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "little_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import little_cli.config as config_mod
    ...
    # whose tools live in little-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "little_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("LITTLE_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "little_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown LITTLE_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("LITTLE_MODEL", raising=False)
    monkeypatch.setenv("LITTLE_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("LITTLE_MODEL", raising=False)
    monkeypatch.delenv("LITTLE_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/little-test "}}
    )

    assert server._resolve_model() == "nous/little-test"


def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("LITTLE_MODEL", "nous/little-test")
    monkeypatch.setenv("LITTLE_TUI_PROVIDER", "nous")
    monkeypatch.delenv("LITTLE_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/little-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("LITTLE_MODEL", "nous/little-test")
    monkeypatch.delenv("LITTLE_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("LITTLE_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "little_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/little-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("LITTLE_MODEL", "sonnet")
    monkeypatch.delenv("LITTLE_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("LITTLE_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "little_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "little_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "little_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "little_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("LITTLE_YOLO_MODE", "false")
    ...
    assert os.environ.get("LITTLE_YOLO_MODE") == "1"
...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_little_home", tmp_path)
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("LITTLE_EXEC_ASK", raising=False)
    monkeypatch.delenv("LITTLE_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("LITTLE_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["LITTLE_GATEWAY_SESSION"] == "1"
    assert server.os.environ["LITTLE_EXEC_ASK"] == "1"
    assert server.os.environ["LITTLE_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("little_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("little_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("little_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, LITTLE_INFERENCE_PROVIDER must
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setenv("LITTLE_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "little_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["LITTLE_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set LITTLE_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. LITTLE_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["LITTLE_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["LITTLE_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    agent = Agent()
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setenv("LITTLE_TUI_PROVIDER", "openai-codex")
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("little_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["LITTLE_TUI_PROVIDER"] == "anthropic"
        assert os.environ["LITTLE_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["LITTLE_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors LittleCLI._manual_compress's
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Little TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("little_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "little_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("little_state")
    ...
    monkeypatch.setitem(sys.modules, "little_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_little_home
    # fixture pins LITTLE_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `little model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `little model` and the classic CLI /model picker.
    ...
    with patch(
        "little_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("little_cli.models.provider_model_ids") as live_fetch:
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"little_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "little_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "little_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "little_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_little_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.little/.env
    ...
    with patch.dict(sys.modules, {"little_cli.config": fake}):
...
def _setup_make_agent_mocks(monkeypatch, cfg):
    ...
    monkeypatch.setattr(
        "little_cli.runtime_provider.resolve_runtime_provider",
