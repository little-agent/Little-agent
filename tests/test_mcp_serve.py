"""
Tests for mcp_serve — Little MCP server.
...
def _isolate_little_home(tmp_path, monkeypatch):
    """Redirect LITTLE_HOME to a temp directory."""
    monkeypatch.setenv("LITTLE_HOME", str(tmp_path))
    try:
        import little_constants
        monkeypatch.setattr(little_constants, "get_little_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking little_state schema."""
