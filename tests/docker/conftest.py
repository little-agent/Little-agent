...
Override the image with ``LITTLE_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LITTLE_TEST_IMAGE", "little-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LITTLE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LITTLE_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged little user
# ---------------------------------------------------------------------------
#
# Background: every Little runtime path inside the container drops to UID
# 10000 (the ``little`` user) via ``s6-setuidgid little``. ``docker exec``
...
# fails (PermissionError swallowed) for little, so a test that ran as root
...
# helpers below run every probe under ``-u little`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: little).
