...
Before the s6 migration, ``docker run little-agent/Little-agent gateway
...
crash and a supervised dashboard alongside (when ``LITTLE_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``LITTLE_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``LITTLE_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``little`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``little gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "LITTLE_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "LITTLE_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The LITTLE_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``little gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``little gateway
    ...
    # Count python processes running `little gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*little.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `little gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-little`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``LITTLE_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${LITTLE_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Little Gateway Starting…"
    ...
    assert "⚕" in combined or "Little Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Little Gateway Starting" in file_contents, (
