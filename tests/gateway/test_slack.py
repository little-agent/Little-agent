...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.little."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /little, /btw, /stop,
        ...
        for expected in ("/little", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@little`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@little"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@little")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /little subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /little btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_little_prefix_still_works(self, adapter):
        """Backward compat: /little btw foo must still route to /btw foo.

        Old workspace manifests only declared /little as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_little_freeform_question(self, adapter):
        """/little <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/little",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_little_slash_stashes_context(self, adapter):
        """Legacy /little <subcommand> also stashes context."""
        command = {
            "command": "/little",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_little_question_does_not_stash_context(self, adapter):
        """Free-form /little <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/little",
