...
def _mention_entity(text, mention="@little_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@little_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@little_bot",
            entities=[_bot_command_entity("/new@little_bot", "/new@little_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@little_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @little_bot", entities=[_mention_entity("hi @little_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@little_bot",
            entities=[_bot_command_entity("/status@little_bot", "/status@little_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="little_bot")

    assert adapter._should_process_message(_group_message("email ops@little_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@little_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @little_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @little_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @little_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @little_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @little_bot")])
    ) is False
