...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "little_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_little_home() / config.yaml`` — point it here.
    monkeypatch.setattr("little_constants.get_little_home", lambda: little_home)
    monkeypatch.setattr("little_cli.config.get_little_home", lambda: little_home)
    return cfg_path
