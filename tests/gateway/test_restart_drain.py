...
def test_load_busy_input_mode_prefers_env_then_config_then_default(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_little_home", tmp_path)
    monkeypatch.delenv("LITTLE_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("LITTLE_GATEWAY_BUSY_INPUT_MODE", "interrupt")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"

    monkeypatch.setenv("LITTLE_GATEWAY_BUSY_INPUT_MODE", "steer")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "steer"

    # Unknown values fall through to the safe default
    monkeypatch.setenv("LITTLE_GATEWAY_BUSY_INPUT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"


def test_load_busy_text_mode_defaults_to_queue_and_allows_interrupt(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_little_home", tmp_path)
    monkeypatch.delenv("LITTLE_GATEWAY_BUSY_TEXT_MODE", raising=False)
    ...
    monkeypatch.setenv("LITTLE_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    monkeypatch.setenv("LITTLE_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"
...
def test_load_restart_drain_timeout_prefers_env_then_config_then_default(
    ...
    monkeypatch.setenv("LITTLE_RESTART_DRAIN_TIMEOUT", "7")
...
async def test_launch_detached_restart_command_uses_setsid(monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_little_bin", lambda: ["/usr/bin/little"])
