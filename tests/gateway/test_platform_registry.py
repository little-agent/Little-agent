...
class TestPlatformsMerge:
    ...
    def test_get_all_platforms_includes_builtins(self):
        from little_cli.platforms import get_all_platforms, PLATFORMS
        merged = get_all_platforms()
        for key in PLATFORMS:
            assert key in merged

    def test_get_all_platforms_includes_plugin(self):
        from little_cli.platforms import get_all_platforms
        from gateway.platform_registry import platform_registry as _reg
    ...
    def test_platform_label_plugin_fallback(self):
        from little_cli.platforms import platform_label
        from gateway.platform_registry import platform_registry as _reg
...
class TestApplyYamlConfigFnDispatch:
    ...
    a tmp LITTLE_HOME, calls load_gateway_config(), and asserts the hook
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        little_home.mkdir()
        (little_home / "config.yaml").write_text(content, encoding="utf-8")
        return little_home
