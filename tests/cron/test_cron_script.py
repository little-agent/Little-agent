...
- Path resolution (absolute, relative to LITTLE_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp LITTLE_HOME."""
    little_home = tmp_path / ".little"
    little_home.mkdir()
    (little_home / "cron").mkdir()
    (little_home / "cron" / "output").mkdir()
    (little_home / "scripts").mkdir()
    monkeypatch.setenv("LITTLE_HOME", str(little_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "LITTLE_DIR", little_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", little_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", little_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", little_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.little/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("LITTLE_SESSION_PLATFORM") is None
        assert os.environ.get("LITTLE_SESSION_CHAT_ID") is None
        assert os.environ.get("LITTLE_SESSION_CHAT_NAME") is None
