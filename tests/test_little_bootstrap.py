"""Tests for little_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Little entry point
(little, little-agent, little-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Little entry point imports little_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported little_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("little_bootstrap", None)
    import little_bootstrap  # noqa: WPS433
    return little_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Little CLI
...
class TestEntryPointsImportBootstrap:
    """Every Little entry point must import little_bootstrap as its
    ...
    # Entry points that invoke Little as a process.  Each one must
    # import little_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "little_cli/main.py",   # little CLI (console_script)
        "run_agent.py",          # little-agent (console_script)
        "acp_adapter/entry.py",  # little-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import little_bootstrap' and that
        ...
        half-finished ``little update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``little_bootstrap`` as a top-level module) leaves little
        ...
        """
        # Resolve relative to the little-agent repo root.  Tests live
        # at tests/test_little_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # little start even when little_bootstrap hasn't been
        ...
        assert first_import_name == "little_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'little_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import little_bootstrap' line to be the first import."
        )
