...
def _isolate(tmp_path, monkeypatch):
    """Isolate LITTLE_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal LittleCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = LittleCLI.__new__(LittleCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["little-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Little' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Little' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Little' minimum context length."""
