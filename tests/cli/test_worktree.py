...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("little/little-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned little/* and pr-* branches."""

    def test_prunes_orphaned_little_branch(self, git_repo):
        """little/little-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "little/little-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "little/little-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "little/little-deadbeef" in result.stdout
        ...
        assert "little/little-deadbeef" in orphaned
        ...
        assert "little/little-deadbeef" not in result.stdout
