...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_little_env(self):
        """Little' own .env inside LITTLE_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: little internal .env is in ~/.little/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.little/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.little/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Little cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        little_home = tmp_path / ".little"
        cache = little_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._little_home_path", return_value=little_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Little cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        little_home = tmp_path / ".little"
        hub = little_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_little_home(self, tmp_path):
        """The env basename guard does not depend on LITTLE_HOME resolution."""
        little_home = tmp_path / ".little"
        little_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        little_home = tmp_path / ".little"
        cache = little_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
