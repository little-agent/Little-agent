...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Little passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("LITTLE_HOME", "/users/alice/.little/profiles/backend-worker")
        monkeypatch.setenv("LITTLE_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "LITTLE_KANBAN_DB",
            "/users/alice/.little/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.little/kanban/boards/smoke"]'
            in cmd
        )
