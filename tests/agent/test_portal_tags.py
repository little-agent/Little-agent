...
def test_little_client_tag_includes_current_version():
    """The client tag must reflect little_cli.__version__ verbatim."""
    from little_cli import __version__
    from agent.portal_tags import little_client_tag

    assert little_client_tag() == f"client=little-client-v{__version__}"


def test_little_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import little_client_tag

    tag = little_client_tag()
    assert tag.startswith("client=little-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import little_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=little-agent" in tags
    assert little_client_tag() in tags
    assert len(tags) == 2
