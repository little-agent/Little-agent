...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Little Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_little_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("LITTLE_HOME", str(tmp_path / ".little"))

    little_env = tmp_path / ".little" / ".env"
    little_env.parent.mkdir(parents=True)
    little_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.little/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
