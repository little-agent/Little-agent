...
to a generic "Your API key was rejected... run little setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "little setup".
    assert "little auth add nous --type oauth" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.little-agent.com" in source
