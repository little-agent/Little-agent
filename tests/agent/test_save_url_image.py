...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate LITTLE_HOME under tmp_path."""
    monkeypatch.setenv("LITTLE_HOME", str(tmp_path / ".little"))
    (tmp_path / ".little").mkdir()

    # Force the constants/image cache helpers to re-read LITTLE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("little_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_little_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under LITTLE_HOME — gateway cleanup
