...
""",
        ...
        from little_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        little_home = tmp_path / ".little"
        little_home.mkdir()
        (little_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("LITTLE_HOME", str(little_home))
        from little_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from little_cli.plugins import PluginContext, PluginManifest, PluginManager
