...
class TestLittleConstantsFallback:
    """Tests for _little_home.py fallback when little_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_little_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _little_home.py with little_constants blocked."""
        monkeypatch.setitem(sys.modules, "little_constants", None)
        spec = importlib.util.spec_from_file_location("_little_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_little_home_env_var(self, monkeypatch, tmp_path):
        """When little_constants is missing, LITTLE_HOME comes from env var."""
        monkeypatch.setenv("LITTLE_HOME", str(tmp_path / "custom-little"))
        module = self._load_helper(monkeypatch)
        assert module.get_little_home() == tmp_path / "custom-little"

    def test_fallback_defaults_to_dot_little(self, monkeypatch):
        """When little_constants is missing and LITTLE_HOME unset, default to ~/.little."""
        monkeypatch.delenv("LITTLE_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_little_home() == Path.home() / ".little"

    def test_fallback_ignores_empty_little_home(self, monkeypatch):
        """Empty/whitespace LITTLE_HOME is treated as unset."""
        monkeypatch.setenv("LITTLE_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_little_home() == Path.home() / ".little"

    def test_fallback_display_little_home_shortens_path(self, monkeypatch):
        """Fallback display_little_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("LITTLE_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_little_home() == "~/.little"

    def test_fallback_display_little_home_profile_path(self, monkeypatch):
        """Fallback display_little_home() handles profile paths under ~/."""
        monkeypatch.setenv("LITTLE_HOME", str(Path.home() / ".little/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_little_home() == "~/.little/profiles/coder"

    def test_fallback_display_little_home_custom_path(self, monkeypatch):
        """Fallback display_little_home() returns full path for non-home locations."""
        monkeypatch.setenv("LITTLE_HOME", "/opt/little-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_little_home() == "/opt/little-custom"

    def test_delegates_to_little_constants_when_available(self):
        """When little_constants IS importable, _little_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_little_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import little_constants
        assert module.get_little_home is little_constants.get_little_home
        assert module.display_little_home is little_constants.display_little_home
