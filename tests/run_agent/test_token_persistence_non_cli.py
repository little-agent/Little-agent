...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    little_state = ModuleType("little_state")
    little_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "little_state", little_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Little"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Little"
