...
class TestToolsetConsistency:
    ...
    def test_little_platforms_share_core_tools(self):
        """All little-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on little-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["little-cli", "little-telegram", "little-discord", "little-whatsapp", "little-slack", "little-signal", "little-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_little_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("little-whatsapp")

    def test_little_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("little-api-server")
