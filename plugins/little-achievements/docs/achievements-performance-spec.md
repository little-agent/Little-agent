# Little Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: little-achievements plugin
...
### `GET /api/plugins/little-achievements/achievements`
...
### `POST /api/plugins/little-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/little-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/little-achievements/overview`
...
- Persist plugin data under:
  - `~/.little/plugins/little-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.little/plugins/little-achievements/scan_checkpoint.json`
...
- `~/.little/plugins/little-achievements/`
