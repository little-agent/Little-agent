...
Config file: `$LITTLE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `little` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `little-{identity}` → `little-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Little profile:

```json
{
  "container_tag": "little-{identity}"
}
```

For a profile named `coder`, this resolves to `little-coder`. The default profile resolves to `little-default`. Without `{identity}`, all profiles share the same container.
