...
def _load_supermemory_config(little_home: str) -> dict:
    config = _default_config()
    config_path = Path(little_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, little_home: str) -> None:
    config_path = Path(little_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `little memory setup`.
        # All other options are documented for $LITTLE_HOME/supermemory.json
    ...
    def save_config(self, values, little_home):
        ...
        _save_supermemory_config(sanitized, little_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from little_constants import get_little_home
        self._little_home = kwargs.get("little_home") or str(get_little_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._little_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "little", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "little_memory", "target": target, "type": "explicit_memory"},
