...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/little>
...
```bash
little honcho setup    # full interactive wizard (cloud or local)
little memory setup    # generic picker, also works
```

Or manually:
```bash
little config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.little/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$LITTLE_HOME/honcho.json` | Profile-local (isolated Little instances) |
| 2 | `~/.little/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Little profile: `little` (default) or `little.<profile>`.
...
**Deployment shapes** (`little honcho setup` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Little to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Little session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`little-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`little`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-little-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening little in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Little session ID (timestamp + hex). Every `little` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Little profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`little`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Little profile: `little` (default) or `little.<profile>` (e.g. `little -p coder` → host key `little.coder`).
...
| `LITTLE_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `little honcho setup` | Full interactive setup wizard |
| `little honcho status` | Show resolved config for active profile |
| `little honcho enable` / `disable` | Toggle Honcho for active profile |
| `little honcho mode <mode>` | Change recall or observation mode |
| `little honcho peer --user <name>` | Update user peer name |
| `little honcho peer --ai <name>` | Update AI peer name |
| `little honcho tokens --context <N>` | Set context token budget |
| `little honcho tokens --dialectic <N>` | Set dialectic max chars |
| `little honcho map <name>` | Map current directory to a session name |
| `little honcho sync` | Create host blocks for all Little profiles |
