...
Handles: little honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "little" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Little profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'little honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'little honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `little update` -- no output, no exceptions.
    """
    try:
        from little_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Little profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $LITTLE_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_little_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        little_host.get("pinUserPeer"),
        little_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in little_host:
        aliases_src = little_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in little_host:
        prefix_src = little_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(little_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Little persistent cross-session memory.")
    ...
    little_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'little honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = little_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        little_host["peerName"] = new_peer

    current_ai = little_host.get("aiPeer") or cfg.get("aiPeer", "little")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        little_host["aiPeer"] = new_ai

    current_workspace = little_host.get("workspace") or cfg.get("workspace", "little")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        little_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, little_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = little_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(little_host)
        little_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{little_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            little_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            little_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'little honcho status' to inspect.")
        ...
        if existing_aliases:
            little_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = little_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        little_host["observationMode"] = new_obs
    else:
        little_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(little_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        little_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        little_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = little_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        little_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = little_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        little_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                little_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(little_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            little_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        little_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        little_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        little_host["dialecticReasoningLevel"] = new_reasoning
    else:
        little_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = little_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        little_host["sessionStrategy"] = new_strat

    little_host["enabled"] = True
    little_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        little_config = load_config()
        little_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(little_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: little config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    little honcho status     -- show full config")
    print("    little honcho mode       -- change recall/observation mode")
    print("    little honcho tokens     -- tune context and dialectic budgets")
    print("    little honcho peer       -- update peer names")
    print("    little honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Little profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: little honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'little honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: little honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        little = hosts.get(_host_key(), {})
        user = little.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = little.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = little.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = little.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Little' identity in Honcho. Seed with 'little honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: little honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: little honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = little.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = little.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = little.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Little asks Honcho's AI peer a question")
        ...
        print("\n  Set with: little honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'little honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'little honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    little honcho identity --show        — show both peer representations")
        print("    little honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Little + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Little\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Little persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  little honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'little honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'little honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  little honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'little'")
        print("  with Honcho configured and no prior session history.")
        print("  (Little calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    little honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'little honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Little, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'little honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    little honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  little honcho identity <path/to/SOUL.md>")
    ...
    print("    Little:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Little:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Little:   Honcho observes every message and updates representations")
    ...
    print("    Little:   per-session by default — each run gets its own session")
    print("              Map a custom name:  little honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. little honcho setup              — configure API key (required)")
        print("  2. little honcho migrate            — re-run this walkthrough")
    else:
        print("  1. little honcho status             — verify Honcho connection")
        print("  2. little                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. little honcho identity --show    — verify AI peer representation")
        print("  4. little honcho tokens             — tune context and dialectic budgets")
        print("  5. little honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'little memory setup'...\n")
        from little_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``little honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``little honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to little memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Little Honcho",
    )
