...
``{title, url, description, position}`` rows every other Little web
provider produces.
...
prefers Little-managed xAI Grok OAuth (via ``little auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.little/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``little tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `little auth` to sign in with "
