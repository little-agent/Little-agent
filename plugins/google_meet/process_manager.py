...
session-scoped state file under ``$LITTLE_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $LITTLE_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_little_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this little install, leave it first —
    ...
    env = os.environ.copy()
    env["LITTLE_MEET_URL"] = url
    env["LITTLE_MEET_OUT_DIR"] = str(out)
    env["LITTLE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["LITTLE_MEET_HEADED"] = "1"
    if auth_state:
        env["LITTLE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["LITTLE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if LITTLE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["LITTLE_MEET_MODE"] = mode
    if realtime_model:
        env["LITTLE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["LITTLE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["LITTLE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["LITTLE_MEET_REALTIME_KEY"] = realtime_api_key
