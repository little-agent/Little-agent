...
Let the little agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where little runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `little meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `little meet` CLI |
...
| `cli.py` | `little meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LITTLE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `little meet node {run,list,approve,remove,status,ping}` |
...
little meet install                                      # pip + Chromium
little meet setup                                        # preflight
little meet auth                                         # optional
little meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
little meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.little/.env
little meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
little meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
little meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.little/.env
little meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, little will **not** switch your system audio input automatically — the
...
little plugins enable google_meet
little meet node run --display-name my-mac --host 0.0.0.0 --port 18789
