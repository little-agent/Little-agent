"""langfuse — Little plugin for Langfuse observability.

Traces Little conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Little plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``little plugins enable
observability/langfuse``, or by checking the box in the interactive
``little plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.little/.env):
  LITTLE_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  LITTLE_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  LITTLE_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  LITTLE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  LITTLE_LANGFUSE_RELEASE     - release/version tag
  LITTLE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  LITTLE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  LITTLE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("LITTLE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Little plugin system —
    ...
    public_key = _env("LITTLE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("LITTLE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("LITTLE_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("LITTLE_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset LITTLE_LANGFUSE_PUBLIC_KEY / LITTLE_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("LITTLE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("LITTLE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("LITTLE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("LITTLE_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid LITTLE_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("LITTLE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Little turn",
                tags=["little", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Little branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Little also has a turn-scoped
    ...
    # messages is a list only for legacy Little branches that fired
    # pre_llm_call with API messages directly. Current Little fires
...
def register(ctx) -> None:
    ...
    # Little versions.  pre_api_request / post_api_request fire per API
