...
Auto-tracks and cleans up ephemeral files created during Little Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LITTLE_HOME` and `/tmp/little-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LITTLE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LITTLE_HOME` or `/tmp/little-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LITTLE_HOME/disk-cleanup/` is itself excluded
- `$LITTLE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
