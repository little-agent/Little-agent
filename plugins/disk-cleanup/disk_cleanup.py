"""disk_cleanup — ephemeral file cleanup for Little Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under LITTLE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly LITTLE_HOME and /tmp/little-*
Never touches: ~/.little/logs/ or any system directory.
"""
...
try:
    ...
    def get_little_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("LITTLE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".little").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$LITTLE_HOME/logs/``."""
    return get_little_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$LITTLE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under LITTLE_HOME or ``/tmp/little-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    little_home = get_little_home()
    try:
        path.resolve().relative_to(little_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/little-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("little-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside LITTLE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under LITTLE_HOME (but leave LITTLE_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "little-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(little_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == little_home:
                continue
            try:
                rel_parts = dirpath.relative_to(little_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    little_home = get_little_home()
    try:
        rel = path.resolve().relative_to(little_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "little-agent",
        }:
            return None
        ...
        # Path isn't under LITTLE_HOME (e.g. /tmp/little-*) — fall through.
        pass
