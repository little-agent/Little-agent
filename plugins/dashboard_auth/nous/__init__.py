...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      LITTLE_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      LITTLE_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.little-agent.com``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``little-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``little_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set LITTLE_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via LITTLE_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.little-agent.com"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"little_session_pkce\"]``
        ...
        cookie_payload = {
            "little_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between LITTLE_DASHBOARD_PORTAL_URL / LITTLE_DASHBOARD_OAUTH_CLIENT_ID
...
def _load_config_oauth_section() -> dict:
    ...
    """
    try:
        from little_cli.config import cfg_get, load_config
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``LITTLE_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("LITTLE_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``LITTLE_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("LITTLE_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``LITTLE_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set LITTLE_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``little_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "LITTLE_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Little Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"LITTLE_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
