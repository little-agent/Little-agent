"""SimpleX Chat platform adapter (Little plugin).
...
This adapter ships as a Little platform plugin under
``plugins/platforms/simplex/``. The Little plugin loader scans the
...
discoverable and `little setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Little — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"little-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``little setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.little/.env`` via ``little_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("little_cli.config not available; set SIMPLEX_* vars manually in ~/.little/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `little gateway status` without
