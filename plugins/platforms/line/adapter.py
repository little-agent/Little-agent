"""
LINE Messaging API platform adapter for Little Agent.
...
to Little Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``LITTLE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            little_home = Path.home().joinpath(".little").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``little status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``little status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``little setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.little/.env`` via ``little_cli.config``.
    """
    print()
    ...
    try:
        from little_cli.config import get_env_var, set_env_var
    except ImportError:
        print("little_cli.config not available; set LINE_* vars manually in ~/.little/.env")
        return
