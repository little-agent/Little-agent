...
# ``model_tools`` import time, so ``little status``, ``little chat``, etc.
...
# ``little_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Little is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a little MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LITTLE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _little_home = _get_little_home()
        except (ModuleNotFoundError, ImportError):
            _little_home = _Path.home() / ".little"
        self._thread_count_store = _ThreadCountStore(
            _little_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Little is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LITTLE_HOME", str(_Path.home() / ".little"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LITTLE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a little MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Little is thinking…' marker message.
        ...
        "Little is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Little is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Little is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Little is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Little is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``little setup``.

    The setup wizard at ``little_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.little/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. little-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.little/.env")
    print_info("Restart the gateway: little gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``little cron`` running as a
    separate process from ``little gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Little plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'little-agent[google_chat]'",
        ...
        # the "Little is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Little is thinking…' marker message "
