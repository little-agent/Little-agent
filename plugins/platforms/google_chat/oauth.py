...
- Per-user tokens (keyed by sender email):
    ``${LITTLE_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${LITTLE_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${LITTLE_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${LITTLE_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${LITTLE_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's LITTLE_HOME helper so the token follows the user's
# profile (e.g. tests can override via LITTLE_HOME=/tmp/...).
try:
    from little_constants import display_little_home, get_little_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where little_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _little_home.py shim).
...
def _little_home() -> Path:
    """Resolve LITTLE_HOME at call time (NOT module import).

    Tests and ``LITTLE_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.little/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _little_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _little_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _little_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _little_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _little_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install little-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'little-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into LITTLE_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_little_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_little_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Little (native attachment delivery)"
    )
