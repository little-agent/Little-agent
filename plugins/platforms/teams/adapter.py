"""
Microsoft Teams platform adapter for Little Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Little"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        little_action = data.get("little_action", "")
        session_key = data.get("session_key", "")

        if not little_action or not session_key:
        ...
        choice = choice_map.get(little_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="little_approve",
                    data={**btn_data_base, "little_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="little_approve",
                    data={**btn_data_base, "little_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="little_approve",
                    data={**btn_data_base, "little_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="little_approve",
                    data={**btn_data_base, "little_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Little\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.little/.env")
