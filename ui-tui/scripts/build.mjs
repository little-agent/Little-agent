...
await build({
  ...
  // Skip the prebuilt @little/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@little/ink': resolve(root, 'packages/little-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The little_cli launcher
