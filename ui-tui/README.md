...
React + Ink terminal UI for Little. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LITTLE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.little/.little_history` or under `LITTLE_HOME`.
...
```text
ui-tui/
  packages/little-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      little-ink.d.ts    type declarations for @little/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent LittleCLI subprocess for slash commands
```
