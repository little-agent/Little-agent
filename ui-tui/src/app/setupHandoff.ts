import type { RunExternalProcess } from '@little/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`little ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching little: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`little ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
