...
// Mirrors @little/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @little/ink is bundled separately from packages/.
...
// Terminal. Explicit LITTLE_TUI_THEME / COLORFGBG signals above still win,
...
// `LITTLE_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `LITTLE_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `LITTLE_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `LITTLE_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Little palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.LITTLE_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.LITTLE_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.LITTLE_TUI_BACKGROUND ?? '')
