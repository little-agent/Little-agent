...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LITTLE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Little on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LITTLE_TUI_TRUECOLOR=1.
