...
There are two ways to add a platform to the Little gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.little/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Little code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `little gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `little_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Little core.
...
And add it to the `little-gateway` composite:

```python
"little-gateway": {
    "includes": [..., "little-your-platform"]
}
```
...
## 12. Status Display (`little_cli/status.py`)
...
## 13. Gateway Setup Wizard (`little_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ little_cli/ toolsets.py \
  --include="*.py" -l | sort -u
