"""
Email platform adapter for the Little gateway.

Allows users to interact with Little by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "little-agent" "version" "{_little_version}" '
            '"vendor" "little-agent" '
            '"support-email" "noreply@little-agent.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Little Agent")
        ...
        msg_id = f"<little-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
