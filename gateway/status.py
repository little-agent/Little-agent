...
The PID file lives at ``{LITTLE_HOME}/gateway.pid``.  LITTLE_HOME defaults to
``~/.little`` but can be overridden via the environment variable.  This means
separate LITTLE_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting LITTLE_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("LITTLE_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "little" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Little gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different LITTLE_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``little.service`` + ``little-
# gateway.service``).
#
