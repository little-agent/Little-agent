...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/little/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when little
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# little process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main little process, the dashboard,
...
# Non-root user for runtime; UID can be overridden via LITTLE_UID at runtime
RUN useradd -u 10000 -m -d /opt/data little
...
#
# ui-tui/packages/little-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/little-ink/ ui-tui/packages/little-ink/
...
# lock on the @little/ink entry, tripped the TUI launcher's
...
# ---------- Permissions ----------
# Make install dir world-readable so any LITTLE_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the little user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# little_cli/main.py succeeds (see #18800). /opt/little/web is build-time
# only (LITTLE_WEB_DIST points at little_cli/web_dist) and is intentionally
# not chowned here.
# The .venv MUST remain little-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/little && \
    chown -R little:little /opt/little/.venv /opt/little/ui-tui /opt/little/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the little user via
# `s6-setuidgid little` in its run script. If LITTLE_UID is unset, services
# run as the default little user (UID 10000).

# ---------- Link little-agent itself (editable) ----------
...
# container always returns nothing — meaning `little dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the LITTLE_GIT_SHA build-arg to
# /opt/little/.little_build_sha at build time, and have
# little_cli/build_info.py read it at runtime.  Both `little dump` and
...
ARG LITTLE_GIT_SHA=
RUN if [ -n "${LITTLE_GIT_SHA}" ]; then \
        printf '%s\n' "${LITTLE_GIT_SHA}" > /opt/little/.little_build_sha && \
        chown little:little /opt/little/.little_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-little + dashboard.
...
# `exec little`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $LITTLE_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/little/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-little-setup && \
    chmod +x /etc/cont-init.d/01-little-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV LITTLE_WEB_DIST=/opt/little/little_cli/web_dist
ENV LITTLE_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> little ...` they default to root, and any file the
# command writes under $LITTLE_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/little/bin/little, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid little`
# when invoked as root. Non-root callers (supervised processes,
# `--user little`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/little/.venv/bin/little). See the shim source for
# the opt-out env var (LITTLE_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/little-exec-shim.sh /opt/little/bin/little

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> little ...` and any
# subprocess that doesn't activate the venv first still find little.
#
# /opt/little/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/little/bin:/opt/little/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. little
# subcommand vs. no-args), drops to the little user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/little/docker/main-wrapper.sh" ]
CMD [ ]
