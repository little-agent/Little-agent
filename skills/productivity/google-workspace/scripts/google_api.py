#!/usr/bin/env python3
"""Google Workspace API CLI for Little Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Little-facing JSON contract and falls back to the Python client
...
from _little_home import get_little_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LITTLE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Little Agent")
