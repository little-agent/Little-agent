#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Little Agent.
...
# Ensure sibling modules (_little_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _little_home import display_little_home, get_little_home

LITTLE_HOME = get_little_home()
TOKEN_PATH = LITTLE_HOME / "google_token.json"
CLIENT_SECRET_PATH = LITTLE_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = LITTLE_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Little profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'little-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Little home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_little_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Little")
