...
## 5. Little Configuration

### Config location

`$LITTLE_HOME/config.yaml` (defaults to `~/.little/config.yaml` when `LITTLE_HOME` is unset)
...
Restart the Little session for changes to take effect. The MCP connection is
