...
LITTLE_HOME_DIR="${LITTLE_HOME:-$HOME/.little}"
LITTLE_CFG="${LITTLE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Little config has twozero_td MCP entry ──
if [[ ! -f "$LITTLE_CFG" ]]; then
    echo -e " ${FAIL} Little config not found at ${LITTLE_CFG}"
    manual_steps+=("Create ${LITTLE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LITTLE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Little config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Little config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LITTLE_CFG} manually"); }
    manual_steps+=("Restart Little session to pick up config change")
fi
