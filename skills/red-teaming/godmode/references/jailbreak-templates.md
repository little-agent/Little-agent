...
## 5. GODMODE FAST — Little 4 405B (Zero Refusal)

**Model:** `little-agent/little-4-405b`
**Strategy:** Little models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Little Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.little/config.yaml`:
