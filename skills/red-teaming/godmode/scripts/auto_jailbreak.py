...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LITTLE_HOME", os.path.expanduser("~/.little")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LITTLE_HOME", Path.home() / ".little")) / "skills" / "red-teaming" / "godmode"
...
LITTLE_HOME = Path(os.getenv("LITTLE_HOME", Path.home() / ".little"))
CONFIG_PATH = LITTLE_HOME / "config.yaml"
PREFILL_PATH = LITTLE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Little models — already uncensored, just needs clean prompt
    "little": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "little" in model_lower or "nous" in model_lower:
        return "little"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Little config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.little/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Little for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Little for changes to take effect.")
