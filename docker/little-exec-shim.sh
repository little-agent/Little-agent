#!/bin/sh
# shellcheck shell=sh
# /opt/little/bin/little — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `little` user (UID 10000). When an operator runs `docker exec <c> little ...`
# the default UID is root (0), and any file the command writes under
# $LITTLE_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> little login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Little is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> little
...
# Fix
# ---
# This shim sits at /opt/little/bin/little and is placed earliest on PATH.
# When invoked as root, it drops to the little user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $LITTLE_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user little`, kanban subagents, etc. — it short-circuits
...
# (/opt/little/.venv/bin/little), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set LITTLE_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the little CLI. Default is to drop.

set -e

REAL=/opt/little/.venv/bin/little

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "little-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user little`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${LITTLE_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the little user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "little-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "little-shim: re-run with --user little or set LITTLE_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the little user's home before dropping privileges. Without
