...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Little starts and finds ``~/.openclaw/``.

    Points users at ``little claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``little claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Little, run "
        "`little claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `little claw cleanup` (renames it to ~/.openclaw.pre-migration — "
