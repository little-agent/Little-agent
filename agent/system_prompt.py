...
upstream prefix cache warm.  See ``little-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Little never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want LITTLE_HOME persona while keeping
    ...
    # Pointer to the little-agent skill + docs for user questions about Little itself.
    stable_parts.append(LITTLE_AGENT_HELP_GUIDANCE)
    ...
    # LITTLE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Little profile the agent is running
    # under so it doesn't conflate ~/.little/skills/ (default profile) with
    # ~/.little/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Little profile: default. Other profiles (if any) live "
            "under ~/.little/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Little profile: {active_profile}. This session reads "
            f"and writes ~/.little/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.little/skills/, ~/.little/plugins/, "
            f"~/.little/cron/, ~/.little/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the little-agent install
        ...
        if getattr(agent, "scanner", None):
            try:
                _scanner_context = agent.scanner.build_llm_context(max_tokens=3000)
                if _scanner_context:
                    context_parts.append(_scanner_context)
            except Exception:
                pass
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Little never rebuilds or reinjects parts of it
