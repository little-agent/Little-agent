...
as ``kind: backend``) or ``~/.little/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``little tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$LITTLE_HOME/cache/videos/``, creating parents as needed."""
    from little_constants import get_little_home

    path = get_little_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$LITTLE_HOME/cache/videos/``.
