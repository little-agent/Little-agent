...
Plugins built on Little Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Little
...
:class:`~little_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Little' main config
...
handles every provider, fallback chain, and per-task override Little
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`little_cli.plugins.PluginContext`
