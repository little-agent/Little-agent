...
Every Little request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Little Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=little-agent",
        "client=little-client-v<__version__>",
    ]

The version is sourced live from ``little_cli.__version__`` so it auto-aligns
...
``little_cli.__version__`` is the canonical source of truth.
"""
...
def _little_version() -> str:
    """Return the current Little release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``little_cli`` cannot be imported (should
...
def little_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=little-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=little-client-v{_little_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=little-agent", little_client_tag()]
