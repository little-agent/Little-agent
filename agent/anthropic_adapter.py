"""Anthropic Messages API adapter for Little Agent.

Translates between Little's internal OpenAI-style message format and
...
# Little effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Little historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Little)
    ...
    # 1. Little-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Little configs.
...
# ── Little-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.little/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_LITTLE_OAUTH_FILE = get_little_home() / ".anthropic_oauth.json"
...
def run_little_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Little-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Little with your Claude Pro/Max subscription.")
    print()
...
def read_little_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Little-managed OAuth credentials from ~/.little/.anthropic_oauth.json."""
    if _LITTLE_OAUTH_FILE.exists():
        try:
            data = json.loads(_LITTLE_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Little OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See little-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See little-agent#13848 (Kimi) and
    little-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Little Agent", "Claude Code")
                text = text.replace("Little agent", "Claude Code")
                text = text.replace("little-agent", "claude-code")
                text = text.replace("Little Agent Team", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Little surfaces in its CLI. We
