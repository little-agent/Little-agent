...
def _little_home_path() -> Path:
    """Resolve the active LITTLE_HOME (profile-aware) without circular imports."""
    try:
        from little_constants import get_little_home  # local import to avoid cycles
        return get_little_home()
    except Exception:
        return Path(os.path.expanduser("~/.little"))


def _little_root_path() -> Path:
    """Resolve the Little root dir (always the parent of any profile, never per-profile)."""
    try:
        from little_constants import get_default_little_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    little_home = _little_home_path()
    little_root = _little_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(little_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(little_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved LITTLE_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("LITTLE_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Little control-plane files: block both the ACTIVE profile's view
    # (little_home) AND the global root view. Without the root pass, a
    ...
    little_dirs = []
    for base in (_little_home_path(), _little_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in little_dirs:
                little_dirs.append(real)
        except Exception:
            continue

    for base_real in little_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Little path.

    Three categories are blocked:

      * Internal Little cache files under ``LITTLE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under LITTLE_HOME and the global Little
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.little/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active LITTLE_HOME (profile-aware) AND the global
    # Little root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (LITTLE_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    little_dirs: list[Path] = []
    for base in (_little_home_path(), _little_root_path()):
        try:
            real = base.resolve()
            if real not in little_dirs:
    ...
    for hd in little_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Little cache file "
    ...
    for hd in little_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Little credential store "
    ...
    for hd in little_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Little MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Little MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Little profiles are separate LITTLE_HOME dirs under
...
#
# Reference: May 2026 incident where a little-security profile session
# edited skills under both ``~/.little/profiles/little-security/skills/``
# AND ``~/.little/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under LITTLE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from LITTLE_HOME.
    ...
    """
    try:
        home_real = _little_home_path().resolve()
        root_real = _little_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Little scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Little entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Little profile {info['target_profile']!r}, but the "
