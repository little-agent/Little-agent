...
from little_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from little_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via LITTLE_CODEX_TTFB_TIMEOUT_SECONDS and
    # LITTLE_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    _ttfb_timeout = _env_float("LITTLE_CODEX_TTFB_TIMEOUT_SECONDS", 12.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("LITTLE_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 25_000.0)
        _ttfb_strict = os.environ.get("LITTLE_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set LITTLE_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("LITTLE_CODEX_TTFB_MAX_SECONDS", 20.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set LITTLE_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "LITTLE_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    _is_nous = "little-agent" in agent._base_url_lower
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "little",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects LITTLE_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in little_cli/config.py).
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        # wins over the LITTLE_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else float(os.getenv("LITTLE_API_TIMEOUT", 1800.0))
        )
        # Read timeout: config wins here too.  Otherwise use
        # LITTLE_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = float(os.getenv("LITTLE_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode LITTLE_STREAM_READ_TIMEOUT.
                ...
                # box is already closed (tool boundary flush).
                    ...
                    try:
                        ...
                        if content and isinstance(content, str):
                            try:
                                from agent.guardrails import run_guardrails_sync, TERMINAL_OUTPUT_GUARDRAILS
                                content = run_guardrails_sync(content, TERMINAL_OUTPUT_GUARDRAILS)
                            except Exception:
                                pass
    ...
    def _call():
        ...
        _max_stream_retries = int(os.getenv("LITTLE_STREAM_RETRIES", 2))
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = float(os.getenv("LITTLE_STREAM_STALE_TIMEOUT", 180.0))
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set LITTLE_STREAM_STALE_TIMEOUT.
