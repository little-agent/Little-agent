...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Little with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.little/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Little
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``little tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``little setup`` for
