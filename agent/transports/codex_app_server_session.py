...
Owns one Codex thread per Little session. Drives `turn/start`, consumes
...
# Little' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_LITTLE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Little/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Little session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _LITTLE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("LITTLE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="little",
            client_title="Little Agent",
            client_version=_get_little_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Little' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Little' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own little-tools callback we
            # auto-accept — the user already approved Little' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Little approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Little returns 'once', 'session', 'always', or 'deny'.
...
def _get_little_version() -> str:
    """Best-effort Little version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("little-agent")
