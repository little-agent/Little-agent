...
from little_constants import get_little_home, get_skills_dir, is_wsl
...
_LITTLE_MD_NAMES = (".little.md", "LITTLE.md")


def _find_little_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.little.md`` or ``LITTLE.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Little agent, an intelligent AI assistant created by Little Agent Team. "
...
LITTLE_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Little agent "
    "itself, load the `little-agent` skill with skill_view(name='little-agent') "
    "before answering. Docs: https://little-agent.little-agent.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.little/kanban.db`. Your task id is in "
    "`$LITTLE_KANBAN_TASK`; your workspace is `$LITTLE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $LITTLE_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `little kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Little WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Little itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Little runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Little itself is running. The host OS, home, and cwd "
                f"of the Little process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Little "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_little_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.little/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("LITTLE_PLATFORM")
        or get_session_env("LITTLE_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Little agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `little-agent` skill "
            "first. It has the actual commands (e.g. `little config set …`, `little tools`, "
            "`little setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: little setup, little setup tools, little setup terminal, little status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from LITTLE_HOME and return its content, or None.
    ...
    """
    try:
        from little_cli.config import ensure_little_home
        ensure_little_home()
    except Exception as e:
        logger.debug("Could not ensure LITTLE_HOME before loading SOUL.md: %s", e)

    soul_path = get_little_home() / "SOUL.md"
...
def _load_little_md(cwd_path: Path) -> str:
    """.little.md / LITTLE.md — walk to git root."""
    little_md_path = _find_little_md(cwd_path)
    if not little_md_path:
        return ""
    try:
        content = little_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = little_md_path.name
        try:
            rel = str(little_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".little.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", little_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .little.md / LITTLE.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from LITTLE_HOME is independent and always included when present.
    ...
    # SOUL.md from LITTLE_HOME only — skip when already loaded as identity
    if not skip_soul:
