...
Storage
-------
Bundles live in ``~/.little/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``LITTLE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``little bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under LITTLE_HOME.

    Honors ``LITTLE_BUNDLES_DIR`` for tests; falls back to
    ``<LITTLE_HOME>/skill-bundles``.
    """
    override = os.environ.get("LITTLE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_little_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `little bundles` CLI subcommand.
# ---------------------------------------------------------------------------
