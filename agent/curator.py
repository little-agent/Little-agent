...
def _state_file() -> Path:
    return get_little_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.little/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``little update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``little curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `little curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Little' background skill CURATOR. This is an "
    ...
    "into ~/.little/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: little-config-*, little-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.little/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.little/logs/curator/``)
    ...
    authored skill data in ``~/.little/skills/``.

    ``ensure_little_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_little_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every little install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: little curator status
        keep an umbrella stable: little curator pin document-tools
    ...
    lines.append("full report: little curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: little curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.little/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.little/skills/.archive/` for "
            "safety and can be restored via `little curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.little/skills/.archive/`. "
            "Restore any via `little curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `little curator restore <name>`")
    lines.append("- All archives live under `~/.little/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `little curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `little curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``little model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `little model` → auxiliary
