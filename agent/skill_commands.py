...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``LITTLE_PLATFORM`` env var and
    ``LITTLE_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("LITTLE_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.little/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.little.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_little_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.little/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.little/skills/`` and any ``skills.external_dirs`` so the
