...
appropriate.  All installs go to a Little-owned bin staging dir,
``<LITTLE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``little lsp
  status``.
...
# ``<LITTLE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def little_lsp_bin_dir() -> Path:
    """Return the Little-owned bin staging dir for LSP servers."""
    home = os.environ.get("LITTLE_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".little")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = little_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = little_lsp_bin_dir().parent  # <LITTLE_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = little_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a little-owned target dir.
    ...
    console script.
    """
    pip_target = little_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = little_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``little lsp status`` CLI to give users a quick
