# OpenAI-Compatible API Server for Little Agent
...
SSE streaming. By exposing this endpoint, little-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  little-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.little/.env`
2. Run `little gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with little-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns little-agent as a model) |
...
Authorization: Bearer little-api-key-here
...
But little-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "little-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured little-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of little-agent's core prompt
...
Once implemented, little-agent works as a drop-in backend for:
