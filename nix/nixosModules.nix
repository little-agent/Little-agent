# nix/nixosModules.nix — NixOS module for little-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: little runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $LITTLE_HOME/.env
# and read by little at startup — no container recreation needed for env changes.
#
# Tool resolution: the little wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.little-agent;
    effectivePackage =
    ...
    little-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-little-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "little-config-attrs";
      description = "Little YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "little-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "little-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'LITTLE_DOC_EOF'\n${value}\nLITTLE_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "little-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/little";
    ...
    # little user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "little-container-entrypoint" ''
      set -eu

      LITTLE_UID="''${LITTLE_UID:?LITTLE_UID must be set}"
      LITTLE_GID="''${LITTLE_GID:?LITTLE_GID must be set}"

      # ── Group: ensure a group with GID=$LITTLE_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$LITTLE_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="little"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$LITTLE_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$LITTLE_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$LITTLE_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$LITTLE_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="little"
        TARGET_HOME="/home/little"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$LITTLE_UID" -g "$LITTLE_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$LITTLE_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$LITTLE_UID:$LITTLE_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure LITTLE_HOME is owned by the target user.
      ...
      if [ -n "''${LITTLE_HOME:-}" ] && [ -d "$LITTLE_HOME" ]; then
        find "$LITTLE_HOME" \! -user "$LITTLE_UID" -exec chown "$LITTLE_UID:$LITTLE_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/little-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/little-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/little ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/little
        chmod 0440 /etc/sudoers.d/little
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$LITTLE_UID" --regid="$LITTLE_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $LITTLE_HOME/.env.
    ...
    # Default: /var/lib/little/workspace → /data/workspace.
  ...
  in {
    options.services.little-agent = with lib; {
      enable = mkEnableOption "Little Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = little-agent;
        description = "The little-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/little";
        description = "State directory. Contains .little/ subdir (LITTLE_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Little config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $LITTLE_HOME/.env at activation time.
          Little reads this file on every startup via load_little_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $LITTLE_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $LITTLE_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `little gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the little user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the little plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Little discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          little_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as little (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in little-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the little CLI to environment.systemPackages and export
          LITTLE_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.little symlink to the service
            stateDir. These users are automatically added to the little group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.little-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the little CLI to system PATH and export LITTLE_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.little/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.LITTLE_HOME = "${cfg.stateDir}/.little";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.little-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the little user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.little-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed little
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure little is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.little        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.little/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.little/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.little/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.little/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.little/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."little-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.little
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.little ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.little ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.little -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.little/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.little/$_subdir"
            chmod 2770 "${cfg.stateDir}/.little/$_subdir"
            find "${cfg.stateDir}/.little/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.little/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.little/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.little/config.yaml
            chmod 0640 ${cfg.stateDir}/.little/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.little/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.little/.managed
          chmod 0644 ${cfg.stateDir}/.little/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.little/.container-mode <<'LITTLE_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    little_bin=${containerDataDir}/current-package/bin/little
    LITTLE_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.little/.container-mode
            chmod 0644 ${cfg.stateDir}/.little/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.little/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.little";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.little" ]; then
                  rm -f "${symlinkPath}"
                  echo "little-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.little -> stateDir/.little for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.little";
                target = "${cfg.stateDir}/.little";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "little-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.little/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.little/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.little/auth.json
              fi
            ''}
          ''}
          ...
          # Little reads $LITTLE_HOME/.env at startup via load_little_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.little/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'LITTLE_NIX_ENV_EOF'
    ${envFileContent}
    LITTLE_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.little/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.little/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.little/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.little-agent = {
          description = "Little Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            LITTLE_HOME = "${cfg.stateDir}/.little";
            LITTLE_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $LITTLE_HOME/.env by the activation script. load_little_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/little"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the little group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.little-agent = {
          description = "Little Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              LITTLE_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              LITTLE_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env LITTLE_UID="$LITTLE_UID" \
                --env LITTLE_GID="$LITTLE_GID" \
                --env LITTLE_HOME=${containerDataDir}/.little \
                --env LITTLE_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/little gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
