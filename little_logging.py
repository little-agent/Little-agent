"""Centralized logging setup for Little Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.little/logs/`` (profile-aware via ``get_little_home()``).
...
from little_constants import get_config_path, get_little_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_little_session_injector", False):
    ...
    _session_record_factory._little_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``little logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "little_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Little logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    little_home
        Override for the Little home directory.  Falls back to
        ``get_little_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_little_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._little_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the little group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from little_cli.config import is_managed
