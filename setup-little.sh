...
# 5. Symlinks the 'little' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/little-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink little into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up little command..."

LITTLE_BIN="$SCRIPT_DIR/venv/bin/little"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LITTLE_BIN" "$COMMAND_LINK_DIR/little"
echo -e "${GREEN}✓${NC} Symlinked little → $COMMAND_LINK_DISPLAY_DIR/little"
...
# ============================================================================
# Seed bundled skills into ~/.little/skills/
# ============================================================================

LITTLE_SKILLS_DIR="${LITTLE_HOME:-$HOME/.little}/skills"
mkdir -p "$LITTLE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.little/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LITTLE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  little status        # Check configuration"
if is_termux; then
    echo "  little gateway       # Run gateway in foreground"
else
    echo "  little gateway install # Install gateway service (messaging + cron)"
fi
echo "  little cron list     # View scheduled jobs"
echo "  little doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m little_cli.main setup
fi
