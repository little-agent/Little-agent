"""ACP auth helpers — detect and advertise Little authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "little-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Little runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Little can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Little.
    ...
    not have Little provider credentials configured yet, so Little always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Little using the currently configured "
