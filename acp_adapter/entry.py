"""CLI entry point for the little-agent ACP adapter.

Loads environment variables from ``~/.little/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from LITTLE_HOME (default ``~/.little``)."""
    from little_cli.env_loader import load_little_dotenv

    little_home = get_little_home()
    loaded = load_little_dotenv(little_home=little_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", little_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="little-acp",
        description="Run Little Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Little version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(little_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import LittleACPAgent  # noqa: F401

    print("Little ACP check OK")


def _run_setup() -> None:
    from little_cli.main import main as little_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "little", "model"]
        little_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``little postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting little-agent ACP adapter")
    ...
    import acp
    from .server import LittleACPAgent
