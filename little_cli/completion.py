"""Shell completion script generation for little CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_little_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Little Agent bash completion
# Add to ~/.bashrc:
#   eval "$(little completion bash)"

_little_profiles() {{
    local profiles_dir="$HOME/.little/profiles"
...
_little_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_little_profiles)" -- "$cur"))
        return
    fi
...
complete -F _little_completion little
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef little
# Little Agent zsh completion
# Add to ~/.zshrc:
...
_little_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.little/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.little/profiles 2>/dev/null)}}")
    fi
...
_little() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_little_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'little command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Little Agent fish completion",
        "# Add to your config:",
        "#   little completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __little_profiles",
        "    echo default",
        "    if test -d $HOME/.little/profiles",
        "        ls $HOME/.little/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c little -f -s p -l profile"
        " -d 'Profile name' -xa '(__little_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__little_profiles)' -d 'Profile name'"
                )
