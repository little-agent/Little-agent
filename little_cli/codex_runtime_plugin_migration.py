"""Migrate Little' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Little' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Little mcp_servers.<n>.command/args/env  → codex stdio transport
  Little mcp_servers.<n>.url/headers       → codex streamable_http transport
  Little mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Little mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Little-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Little' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Little config.")
...
# Little keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Little' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, little_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Little MCP server config to the codex inline-table dict
    ...
    if not isinstance(little_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(little_cfg.get("command"))
    has_url = bool(little_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (LITTLE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Little)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Little' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Little' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Little
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="little-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a LITTLE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed little-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.little`` symlink that happens to live under ``/private/var/folders``
...
def _build_little_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Little' own
    ...
    so a little installed under /opt/, /usr/local/, or a venv all work.
    LITTLE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # LITTLE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_little_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever LITTLE_HOME its launcher sets
    ...
    # — that would override the launcher's LITTLE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("LITTLE_HOME", tmp_path) would
    ...
    little_home = os.environ.get("LITTLE_HOME") or ""
    if little_home and _looks_like_test_tempdir(little_home):
        little_home = ""
    if little_home:
        env["LITTLE_HOME"] = little_home
    # PYTHONPATH passes through so a worktree-launched little finds the
    ...
    env["LITTLE_QUIET"] = "1"
    env["LITTLE_REDACT_SECRETS"] = env.get("LITTLE_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_little_tools: bool = True,
) -> MigrationReport:
    """Translate Little mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        little_config: full ~/.little/config.yaml dict
        ...
        expose_little_tools: when True (default), register Little' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Little for tools
    ...
    little_servers = (little_config or {}).get("mcp_servers") or {}
    if not isinstance(little_servers, dict):
        report.errors.append(
            "mcp_servers in Little config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in little_servers.items():
    ...
    # Inject Little' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Little for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/little_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_little_tools:
        translated["little-tools"] = _build_little_tools_mcp_entry()
        if "little-tools" not in report.migrated:
            report.migrated.append("little-tools")
