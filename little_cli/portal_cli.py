"""``little portal`` — small CLI surface for Nous Portal users.
...
already in ``little auth`` or ``little tools``. It's a discovery + status
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   little auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `little model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `little auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `little portal <subcommand>`."""
    ...
    print("Run `little portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `little portal` on the given argparse subparsers object."""
