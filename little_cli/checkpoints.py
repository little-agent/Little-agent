"""`little checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.little/checkpoints/``.  Actions:

    little checkpoints               # same as `status`
    little checkpoints status        # total size, project count, breakdown
    little checkpoints list          # per-project checkpoint counts + workdir
    little checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    little checkpoints clear [-f]    # nuke the entire base (asks first)
    little checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    little checkpoints
    little checkpoints prune --retention-days 3 --max-size-mb 200
    little checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: little checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``little checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `little checkpoints` → status
