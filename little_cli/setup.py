"""
Interactive setup wizard for Little Agent.
...
from little_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://little-agent.little-agent.com/docs"
...
# display_little_home imported lazily at call sites (stale-module safety during little update)
...
from little_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from little_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Little Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Little using environment variables or config commands:")
    print_info("  little config set model.provider custom")
    print_info("  little config set model.base_url http://localhost:8080/v1")
    print_info("  little config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'little setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from little_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from little_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'little setup')")


def _print_setup_summary(config: dict, little_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'little setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from little_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `little tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from little_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'little setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'little setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'little setup terminal'"))
    ...
    # Spotify (OAuth via little auth spotify — check auth.json, not env vars)
    try:
        from little_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'little setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {little_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('little setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('little setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('little setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('little setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('little setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('little config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('little config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('little config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('little', Colors.GREEN)}              Start chatting")
    print(f"   {color('little gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('little doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `little tools` and `little setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``little model``)
    ...
    provider added to ``little model`` is automatically available here.
    ...
    # Delegate to the shared little model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from little_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: little model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from little_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Little can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Little can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'little setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``little model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from little_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.little/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Little auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via little setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'little setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Little runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into LITTLE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("LITTLE_MAX_ITERATIONS")
    ...
    print_info("  Run `little setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # LITTLE_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Little delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `little update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://little-agent.little-agent.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under LITTLE_HOME, and print
    ...
    """
    try:
        from little_cli.slack_cli import _build_full_manifest
        from little_constants import get_little_home

        manifest = _build_full_manifest(
            bot_name="Little",
            bot_description="Your Little agent on Slack",
        )
        target = Path(get_little_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `little slack manifest --write` anytime to refresh after "
            "Little adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Little delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Little to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from little_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://little-agent.little-agent.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://little-agent.little-agent.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  little config edit")
    print_info("   Open config in your editor:  little config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from little_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Little from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'little setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     little config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_little_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  little gateway")
        else:
            from little_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   little gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   little gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `little setup tools` and `little tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``little_cli.auth`` — lists every supported
    ...
    """
    try:
        from little_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from little_cli.config
    ...
    if section_key == "model":
        ...
        from little_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_little.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_little migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_little", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Little.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Little to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Little at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Little at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Little at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Little at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Little equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Little config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Little.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Little's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(little_home: Path) -> bool:
    ...
    print_info("Little can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: little claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=little_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: little claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Little configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Little config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Little (use little claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``little setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``little setup --portal``)
    that gets a brand-new user from zero to a fully working Little session
    ...
    from little_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Little Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print()
    print_info("  Sign up: https://portal.little-agent.com/manage-subscription")
    print()

    # Skip OAuth if already logged in (don't re-prompt every time the user
    # runs `little setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `little auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `little auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `little model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `little portal status` to inspect routing.")
    print_info("  Run `little` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      little setup           — full or quick (auto-detected)
      little setup model     — just model/provider
      little setup tts       — just text-to-speech
      little setup terminal  — just terminal backend
      little setup gateway   — just messaging platforms
      little setup tools     — just tool configuration
      little setup agent     — just agent settings
    """
    from little_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Little Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Little Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Little Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, little_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Little configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'little setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(little_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Little?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, little_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {little_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'little config edit'")
    ...
    _print_setup_summary(config, little_home)


def _run_first_time_quick_setup(config: dict, little_home, is_existing: bool):
    ...
    the user can customize later via ``little setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'little setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    little setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  little setup gateway")
    print()

    _print_setup_summary(config, little_home)


def _run_quick_setup(config: dict, little_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'little setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Little to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'little setup gateway'.")
