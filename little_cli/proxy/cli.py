"""CLI handlers for the ``little proxy`` subcommand."""
...
from little_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "little proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"little auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Little proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Little proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: little proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``little proxy <subcommand>``."""
    ...
    print(
        "little proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  little proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
