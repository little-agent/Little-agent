...
Reads the user's Nous OAuth state from ``~/.little/auth.json`` through the
...
:func:`little_cli.auth.resolve_nous_runtime_credentials`.
"""
...
# Endpoints inference-api.little-agent.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `little auth add nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `little auth add nous` to re-authenticate."
                )
    ...
    # to little_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
