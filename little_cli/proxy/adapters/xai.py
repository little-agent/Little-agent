...
# xAI's public API is OpenAI-compatible for the endpoints Little commonly
# uses. The Responses endpoint is included because Little' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Little-managed OAuth credentials."""

    auth_hint = "little auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`little auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`little auth reset xai-oauth` or re-authenticate with "
                    "`little auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `little auth add xai-oauth --type oauth`."
            )
