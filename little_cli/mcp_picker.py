"""MCP picker — interactive `little mcp picker` (also the default `little mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``little mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `little plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from little_cli.cli_output import prompt_yes_no
from little_cli.curses_ui import curses_single_select
from little_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Little session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from little_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `little mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: little mcp install <name>    Picker: little mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Little is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Little — run `little update` "
...
def show_catalog() -> None:
    """`little mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`little mcp picker` (and default `little mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`little mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `little mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
