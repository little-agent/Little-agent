"""
Little Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.little/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Little PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing little-agent or little PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Little Agent" comment and following line
                if '# Little Agent' in line or '# little-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('little' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing little
                if 'little' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the little wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "little",
        Path("/usr/local/bin/little"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains little_cli reference)
                content = wrapper.read_text()
                if 'little_cli' in content or 'little-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``little gateway run`` processes
    ...
    try:
        from little_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from little_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``LITTLE_HOME`` and ``LITTLE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\little\git\cmd``, ``%LOCALAPPDATA%\little\git\bin``,
#      ``%LOCALAPPDATA%\little\git\usr\bin``, ``%LOCALAPPDATA%\little\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\little\git\`` and Node to
#      ``%LOCALAPPDATA%\little\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``little dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\little\gateway-service\`` and sometimes
...
def _little_path_markers(little_home: Path) -> list[str]:
    """Path-entry substrings that identify Little-owned User-PATH entries."""
    root = str(little_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare little-agent install dir.
    markers = [root + "\\little-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if LITTLE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "little".  We don't want to catch
    # unrelated entries like "clittle-foo" or "ephermeral", so we look for
    # backslash-little as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(little_home: Path) -> list[str]:
    """Strip Little-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _little_path_markers(little_home)
            kept: list[str] = []
...
def remove_little_env_vars_windows() -> list[str]:
    """Delete LITTLE_HOME and LITTLE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("LITTLE_HOME", "LITTLE_GIT_BASH_PATH"):
                try:
...
def _is_default_little_home(little_home: Path) -> bool:
    """Return True when ``little_home`` points at the default (non-profile) root."""
    try:
        from little_constants import get_default_little_root
        return little_home.resolve() == get_default_little_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LITTLE_HOME directory.

    We shell out to ``little -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LITTLE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m little_cli.main` so we don't depend on a `little`
    #    wrapper that may be half-removed mid-uninstall.
    little_invocation = [_sys.executable, "-m", "little_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                little_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's LITTLE_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.little/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.little/ for future reinstall
    """
    project_root = get_project_root()
    little_home = get_little_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie LITTLE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_little_home(little_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Little Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {little_home / 'config.yaml'}")
    print(f"  Secrets: {little_home / '.env'}")
    print(f"  Data:    {little_home / 'cron/'}, {little_home / 'sessions/'}, {little_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their LITTLE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Little data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Little code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in little_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\little\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(little_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Little-owned PATH entries in User environment")

        log_info("Removing LITTLE_HOME / LITTLE_GIT_BASH_PATH User env vars...")
        removed_env = remove_little_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Little-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing little command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.little/, just remove the little-agent subdir
            if little_home in project_root.parents or project_root.parent == little_home:
    ...
    #     under LITTLE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(little_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(little_home)
    ...
    # 5. Optionally remove ~/.little/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile LITTLE_HOME dirs live under
        ...
        try:
            if little_home.exists():
                shutil.rmtree(little_home)
                log_success(f"Removed {little_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {little_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {little_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/little-agent/Little-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/little-agent/Little-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Little Agent! ⚕")
    print()
