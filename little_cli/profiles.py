"""
Profile management for multiple isolated Little instances.

Each profile is a fully independent LITTLE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.little/profiles/<name>/`` by default.

The "default" profile is ``~/.little`` itself — backward compatible,
zero migration needed.

Usage::

    little profile create coder          # fresh profile + bundled skills
    little profile create coder --clone  # also copy config, .env, SOUL.md, skills
    little profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    little -p coder chat                 # or via flag
    little profile use coder             # set as sticky default
    little profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See little_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.little``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   little-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `little profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `little
...
# `little skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Little infrastructure directories that only the default profile
         (``~/.little``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_little_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.little) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "little-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "little", "default", "test", "tmp", "root", "sudo",
})

# Little subcommands that cannot be used as profile names/aliases
_LITTLE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the little root, NOT to the current LITTLE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LITTLE_HOME points outside
    ``~/.little``, profiles live under ``LITTLE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_little_home() / "profiles"


def _get_default_little_home() -> Path:
    """Return the default (pre-profile) LITTLE_HOME path.

    In standard deployments this is ``~/.little``.
    In Docker/custom deployments where LITTLE_HOME is outside ``~/.little``
    (e.g. ``/opt/data``), returns LITTLE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_little_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Little installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LITTLE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_little_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, little subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _LITTLE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a little subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``little profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Little config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_little_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``little update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.little)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.little/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `little update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`little profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `little update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `little profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `little -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LITTLE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``little profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LITTLE_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.little).\n"
            "To remove everything, use: little uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from little_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LITTLE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LITTLE_HOME")
    try:
        os.environ["LITTLE_HOME"] = str(profile_dir)
        from little_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LITTLE_HOME"] = old_home
        elif "LITTLE_HOME" in os.environ:
            del os.environ["LITTLE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.little/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: little profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LITTLE_HOME.

    Returns ``"default"`` if LITTLE_HOME is not set or points to ``~/.little``.
    Returns the profile name if LITTLE_HOME points into ``~/.little/profiles/<name>``.
    Returns ``"custom"`` if LITTLE_HOME is set to an unrecognized path.
    """
    from little_constants import get_little_home
    little_home = get_little_home()
    resolved = little_home.resolve()

    default_resolved = _get_default_little_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.little itself — its parent is ~/ and its
        # directory name is ".little", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: little profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.little itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.little). "
            "Specify a different name: little profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="little_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"little.{old_name}"
    new_host = f"little.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_little_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LITTLE_HOME path string.

    Called early in the CLI entry point, before any little modules
    are imported, to set the LITTLE_HOME environment variable.
    """
