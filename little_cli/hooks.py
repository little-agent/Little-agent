"""little hooks — inspect and manage shell-script hooks.

Usage::

    little hooks list
    little hooks test <event> [--for-tool X] [--payload-file F]
    little hooks revoke <command>
    little hooks doctor

Consent records live under ``~/.little/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.little/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``little hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: little hooks {list|test|revoke|doctor}")
        print("Run 'little hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.little/config.yaml.")
        print("See `little hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `little hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `little hooks test` and `little hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Little wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `little hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `little hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `little hooks doctor`.")
