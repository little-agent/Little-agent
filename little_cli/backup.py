"""
Backup and import commands for little CLI.

`little backup` creates a zip archive of the entire ~/.little/ directory
(excluding the little-agent repo and transient files).

`little import` restores from a backup zip, overlaying onto the current
LITTLE_HOME root.
"""
...
from little_constants import get_default_little_root, get_little_home, display_little_home
...
_EXCLUDED_DIRS = {
    "little-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to little root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside LITTLE_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Little home directory."""
    little_root = get_default_little_root()

    if not little_root.is_dir():
        print(f"Error: Little home directory not found at {little_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"little-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"little-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_little_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(little_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(little_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(little_root)
    ...
    print(f"\nRestore with: little import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Little backup.
    ...
    # Look for telltale files that a little home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Little backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.little/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a little dir name
        if prefix in {".little", "little"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_little_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (little_root / "config.yaml").exists()
        has_env = (little_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Little configuration.")
        ...
        for member in members:
            ...
            target = little_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(little_root.resolve())
            except ValueError:
        ...
        profiles_dir = little_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # little_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: little profile list  (after installing little)")

        # Guidance
        print()
        if not (little_root / "little-agent").is_dir():
            print("Note: The little-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: little update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  little -p {pname} gateway install")

        print("Done. Your Little configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and little backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to LITTLE_HOME).
...
# — `little update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(little_home: Optional[Path] = None) -> Path:
    home = little_home or get_little_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    little_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(little_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(little_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for little backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_little_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, little_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``little_root`` to ``out_path``.
...
def _pre_update_backup_dir(little_home: Optional[Path] = None) -> Path:
    home = little_home or get_little_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of LITTLE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<LITTLE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``little update``) should continue even if the backup fails.
    """
    little_root = little_home or get_default_little_root()
    if not little_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(little_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, little_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `little claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LITTLE_HOME under ``backups/`` before a
    ``little claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``little import <archive>``.  Writes to
    ``<LITTLE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `little import` and the
