...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or little -c.",
    ...
    "Ctrl+Z suspends Little to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "little -c resumes your most recent CLI session. little -c \"project name\" resumes by title.",
    "little -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "little -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "little chat -t web,terminal enables only specific toolsets for a focused session.",
    "little chat -s github-pr-workflow preloads a skill at launch.",
    "little chat -q \"query\" runs a single non-interactive query and exits.",
    "little chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "little chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "little --yolo bypasses all dangerous command approval prompts for the entire session.",
    "little chat --source telegram tags the session for filtering in little sessions list.",
    "little -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "little doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "little dump outputs a compact setup summary — great for bug reports.",
    "little config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "little config edit opens config.yaml in your default editor.",
    "little config check scans for missing or stale configuration options.",
    "little sessions browse opens an interactive session picker with search.",
    "little sessions stats shows session counts by platform and database size.",
    "little sessions prune --older-than 30 cleans up old sessions.",
    "little skills search react --source skills-sh searches the skills.sh public directory.",
    "little skills check scans installed hub skills for upstream updates.",
    "little skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "little skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "little mcp add github --command npx adds MCP servers from the command line.",
    "little mcp serve runs Little itself as an MCP server for other agents.",
    "little auth add lets you add multiple API keys for credential pool rotation.",
    "little completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "little logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "little backup creates a zip backup of your entire Little home directory.",
    "little profile create coder creates an isolated profile that becomes its own command.",
    "little profile create work --clone copies your current config and keys to a new profile.",
    "little update syncs new bundled skills to ALL profiles automatically.",
    "little gateway install sets up Little as a system service (systemd/launchd).",
    "little memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "little webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: little tools disables unused tools, little skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "little models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as little -w).",
    ...
    "LITTLE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Little tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'little profile create coder' creates the 'coder' command.",
    "little profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Little prints a resume command with session ID and stats.",
    "little sessions export backup.jsonl exports all sessions for backup or analysis.",
    "little -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "little skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .little/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: little cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Little runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "little gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Little via a local macOS server.",
    ...
    "Context files (.little.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.little/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Little loads project context from .little.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "little mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "little mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "little mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — little tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.little/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "little chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "little plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "little acp runs Little as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "LITTLE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "little auth add nous or little auth add openai-codex sets up OAuth-based providers.",
    ...
    "little status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.little/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Little your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.little/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Little auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.little/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at LITTLE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "LITTLE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in LITTLE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.little/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'little profile create ls' would be rejected.",
    "little profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "little claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "LITTLE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via little tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.little/.env.",
    "Skills from trusted repos (little-agent) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.little/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Little auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Little config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Little is working.',
    ...
    '/reload reloads ~/.little/.env into the running session — pick up new API keys without restarting.',
    ...
    "LITTLE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.little/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.little/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'little curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "little curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'little curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'little auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'LITTLE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "LITTLE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'little dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'little dashboard --tui embeds the full Little TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.little/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.little/dashboard-plugins/ — no npm build required.',
    ...
    'LITTLE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'LITTLE_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'LITTLE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'LITTLE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.little/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.little/skills/ so drop-in skills appear without restarting the session.',
    ...
    'little -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'little chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'little chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'little chat --ignore-user-config skips ~/.little/config.yaml — reproducible bug reports and CI runs.',
    "little chat --source tool tags programmatic chats so they don't clutter little sessions list.",
    'little dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'little sessions rename <ID> "new title" renames any past session; little sessions delete <ID> removes one.',
    'little import restores a session export or profile archive produced by sessions export or profile export.',
    'little fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'little pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'little setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'little status --deep runs the full health sweep across every component; plain little status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'LITTLE_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'LITTLE_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.little/plugins/ — trust-gated by design.',
    "LITTLE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'LITTLE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'LITTLE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'LITTLE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'LITTLE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'LITTLE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'LITTLE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'LITTLE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'LITTLE_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'LITTLE_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'LITTLE_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Little block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from little sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.little/dashboard-plugins/.',
]
