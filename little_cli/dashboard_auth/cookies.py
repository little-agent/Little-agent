...
Three cookies in play:
  - little_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL)
  - little_session_rt:   the OAuth refresh token
  ...
  - little_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/little) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/little`` covers
...
setter wrote ``__Secure-little_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "little_session_at"
SESSION_RT_COOKIE = "little_session_rt"
PKCE_COOKIE = "little_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /little`` we want ``Path=/little`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/little``)
