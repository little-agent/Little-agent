...
prefix (e.g. ``mission-control.tilos.com/little/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /little`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``LITTLE_DASHBOARD_PUBLIC_URL`` /
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/little"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# LITTLE_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``LITTLE_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("LITTLE_DASHBOARD_PUBLIC_URL", "")
