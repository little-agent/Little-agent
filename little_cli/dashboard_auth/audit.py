...
Profile-aware location: ``$LITTLE_HOME/logs/dashboard-auth.log``.
...
from ``little_constants`` or other little_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$LITTLE_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``little_constants.get_little_home`` semantics: env var wins,
    else ``~/.little``. A local copy avoids an import cycle with the
    middleware which lives below ``little_cli``.
    """
    home = os.environ.get("LITTLE_HOME") or str(Path.home() / ".little")
