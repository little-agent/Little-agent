"""
little fallback — manage the fallback provider chain.
...
Subcommands:
  little fallback [list]   Show the current fallback chain (default when no subcommand)
  little fallback add      Pick provider + model via the same picker as `little model`,
                           then append the selection to the chain
  little fallback remove   Pick an entry to delete from the chain
  little fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.little/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from little_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `little model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  little fallback add")
        print()
        return
    ...
    print("  Docs: https://little-agent.little-agent.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `little model`, then append the selection to the chain."""
    from little_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `little model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `little fallback list` to view, or `little fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``little fallback [subcommand]``."""
