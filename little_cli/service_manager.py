...
the existing module-level functions in little_cli.gateway and
little_cli.gateway_windows directly. This protocol is a thin facade
...
``little gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``little gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged little
    ...
    detection always fails. Since every Little runtime call inside the
    container drops to little via ``s6-setuidgid``, that silent failure
...
# in ``little_cli.gateway`` (systemd/launchd) and ``little_cli.gateway_windows``
...
# active (set via the ``little -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in little_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from little_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from little_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from little_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from little_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in little_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from little_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from little_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from little_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from little_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``little_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from little_cli import gateway_windows
        from little_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `little profile create`
# runs inside the container (Phase 4). Static services (main-little, dashboard)
...
# UID/GID of the in-image ``little`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the little user.
    ...
    (the ``little`` user — UID 10000 — running every Little runtime
    ...
    is dispatched as the little user.
    ...
    with little ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           little:little, 0755 (parent must already exist)
    ``svc_dir/event/``                     little:little, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 little:little, 0755
    ``svc_dir/supervise/event/``           little:little, 03730
    ``svc_dir/supervise/control``          little:little, 0660    (FIFO)
    ...
    slot's supervise/ was little-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _LITTLE_UID, _LITTLE_GID)
        except PermissionError:
            # Running as the little user already — directory is little-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _LITTLE_UID, _LITTLE_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is little-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _LITTLE_UID, _LITTLE_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``little -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`little profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-little, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources LITTLE_HOME (and any extra env) via with-contenv —
             so e.g. ``-e LITTLE_HOME=/data/little`` is honored at run
          ...
          4. Drops to the little user and exec's
             ``little -p <profile> gateway run`` (or just ``little
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``little gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root LITTLE_HOME profile" (the implicit profile that exists at
        the top of $LITTLE_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``little_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$LITTLE_HOME/profiles/default/`` — a
        ...
        ``little_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export LITTLE_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            lines.append("exec s6-setuidgid little little gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid little little -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${LITTLE_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the LITTLE_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e LITTLE_HOME=/data/little``
        gets its logs under /data/little/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``little logs`` reads and what
        ...
        return (
            ...
            f': "${{LITTLE_HOME:=/opt/data}}"\n'
            f'log_dir="$LITTLE_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R little:little "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid little s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with little ownership
            ...
            # the little user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is little-owned (see ``_seed_supervise_skeleton``), and on
