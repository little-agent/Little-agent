...
``$LITTLE_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-little-setup
(the stage2 hook) has chowned the volume and seeded $LITTLE_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$LITTLE_HOME``,
    not under ``profiles/``). The dispatcher in ``little_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``little gateway start`` (no ``-p``) targets. Without it,
    bare ``little gateway start`` inside the container would land on
    ...
    ``$LITTLE_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        little_home: The container's LITTLE_HOME (typically /opt/data).
            Profiles live under ``<little_home>/profiles/<name>/``;
            the default profile lives at ``<little_home>`` itself.
    ...
    # named profiles).
    default_prior_state = _read_prior_state(little_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(little_home)
    ...
    profiles_root = little_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `little profile create` (config.yaml
            # is not — that comes later via `little setup`). Use it as the
            ...
            # via ``little -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(little_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `little -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit little
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the little user) won't hit EACCES. See
...
def _write_reconcile_log(
    little_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $LITTLE_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    little_home = Path(os.environ.get("LITTLE_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        little_home=little_home, scandir=scandir,
    )
    for a in actions:
        print(
