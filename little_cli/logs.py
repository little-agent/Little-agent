"""``little logs`` — view and filter Little log files.
...
Usage examples::
    ...
    little logs                    # last 50 lines of agent.log
    little logs -f                 # follow agent.log in real time
    little logs errors             # last 50 lines of errors.log
    little logs gateway -n 100    # last 100 lines of gateway.log
    little logs --level WARNING    # only WARNING+ lines
    little logs --session abc123   # filter by session ID substring
    little logs --component tools  # only tool-related lines
    little logs --since 1h         # lines from the last hour
    little logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_little_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Little runs — try 'little chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_little_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_little_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_little_home()}/logs/")
        return

    print(f"Log files in {display_little_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'little chat' to generate logs)")
