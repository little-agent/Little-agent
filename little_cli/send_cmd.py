"""CLI subcommand: ``little send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"little send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"little send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"little send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"little send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"little send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `little gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.little/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_little_env() -> None:
    """Populate ``os.environ`` from ``~/.little/.env`` AND bridge top-level
    ...
    startup that ``little send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.little/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.little/config.yaml`` into
       ...
       via ``little config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``little send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.little/.env and ~/.little/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "little send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  little send --to telegram \"hello\"\n"
            "  little send --to discord:#ops --file report.md\n"
            "  little send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "little send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `little send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Little "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.little/.env + ~/.little/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `little send --list telegram`.",
    )
