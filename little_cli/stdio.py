...
on Japanese locales, etc.).  Little's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``little_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Little is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``LITTLE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("LITTLE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Little-managed Git install directories so
    ...
    # already-running shells don't see the change — which means little
    ...
    # install without Little-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Little's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Little's
    ``little config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Little (User env var in Windows Settings, or
    export in a PowerShell profile) and Little picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Little-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\little\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``little`` from right after install) retain their old PATH.

    Any subprocess Little spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Little-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "little", "git", "cmd"),
        os.path.join(local_appdata, "little", "git", "bin"),
        os.path.join(local_appdata, "little", "git", "usr", "bin"),
        # Little venv Scripts directory — host of the little.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes little, but harmless to include.
        os.path.join(local_appdata, "little", "little-agent", "venv", "Scripts"),
