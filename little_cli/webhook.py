"""little webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.little/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _little_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: little gateway run
"""
...
def webhook_command(args):
    """Entry point for 'little webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: little webhook {subscribe|list|remove|test}")
        print("Run 'little webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (little gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: little webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from little webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (little gateway run)")
