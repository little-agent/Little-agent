"""
Status command for little CLI.

Shows the status of all Little Agent components.
"""
...
from little_cli.auth import AuthError, resolve_provider
from little_cli.colors import Colors, color
from little_cli.config import get_env_path, get_env_value, get_little_home, load_config
from little_cli.models import provider_label
from little_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from little_cli.nous_subscription import get_nous_subscription_features
from little_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``little config``'s
...
def show_status(args):
    """Show status of all Little Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Little Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: little auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: little model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: little auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: little auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: little model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from little_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from little_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   little gateway")
    ...
    jobs_file = get_little_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_little_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'little doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'little setup' to configure", Colors.DIM))
    print()
