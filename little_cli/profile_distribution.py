"""Profile distributions — shareable, packaged Little profiles via git.

A distribution is a Little profile published as a git repository (or
...
* ``little profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``little skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``little profile``, not a parallel tree):

    little profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "little-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            little_requires=str(data.get("little_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.little_requires:
            out["little_requires"] = self.little_requires
        if self.author:
...
def check_little_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Little {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``little profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Little profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Little distribution."
        )

    # Version check up-front so we fail fast
    check_little_requires(manifest.little_requires, little_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.little).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="little_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `little profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`little profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="little_dist_update_") as tmp:
        plan = plan_install(
