"""
Dump command for little CLI.

Outputs a compact, plain-text summary of the user's Little setup
...
from little_cli.config import get_little_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.little_build_sha`` by the Dockerfile's
    ``LITTLE_GIT_SHA`` build-arg (see ``little_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``little dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from little_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(little_home: Path) -> int:
...
def _cron_summary(little_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = little_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"little_home:      {display_little_home()}")
    ...
    toolsets = config.get("toolsets", ["little-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(little_home)}")
    lines.append(f"  skills:             {_count_skills(little_home)}")
