...
``<root>`` is the **shared Little root** (the parent of any active
...
``little -p <profile>`` joins the same board as the dispatcher that
...
the profile's Little home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``LITTLE_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``LITTLE_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``little kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.little``. In Docker / custom
deployments where ``LITTLE_HOME`` points outside ``~/.little`` (e.g.
``/opt/little``), ``<root>`` is ``LITTLE_HOME``. Legacy env-var
overrides still work:

* ``LITTLE_KANBAN_DB`` — pin the database file path directly.
* ``LITTLE_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``LITTLE_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``LITTLE_KANBAN_DB``,
``LITTLE_KANBAN_WORKSPACES_ROOT``, and ``LITTLE_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/little-kanban-v1-spec.pdf`` for the full
...
# ``LITTLE_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``LITTLE_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("LITTLE_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``LITTLE_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("LITTLE_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``little-agent``
...
def kanban_home() -> Path:
    """Return the shared Little root that anchors the kanban board.

    Resolution order:

    1. ``LITTLE_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_little_root()``, which already returns ``<root>``
       when ``LITTLE_HOME`` is ``<root>/profiles/<name>``, and returns
       ``LITTLE_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``LITTLE_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("LITTLE_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from little_constants import get_default_little_root
    return get_default_little_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``little kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``LITTLE_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``little kanban boards
    ...
    """
    env = os.environ.get("LITTLE_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``little kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``LITTLE_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("LITTLE_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``LITTLE_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("LITTLE_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``little kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``LITTLE_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``LITTLE_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("LITTLE_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Little treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``LITTLE_KANBAN_DB`` env → ``LITTLE_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See little_state._WAL_INCOMPAT_MARKERS for detection logic.
                from little_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``little kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``LITTLE_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from little_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``little --skills ...`` alongside the built-in
    ...
    # `little --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``little kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `little kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``little kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``little kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``little kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``little kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``LITTLE_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Little' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Little-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``little kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Little
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``little kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Little profile.
    ...
    if not rows:
        return False
    try:
        from little_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Little profile.
        # `_default_spawn` invokes ``little -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Little
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `little kanban tail` — without
...
def _module_little_argv() -> list[str]:
    """Return the interpreter-bound Little CLI invocation."""
    # ``little_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``little`` package — there is no
    # ``little`` package to import.
    return [sys.executable, "-m", "little_cli.main"]


def _absolute_little_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Little shim."""
...
def _little_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Little executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_little_argv()
    return [_absolute_little_path(path)]


def _resolve_little_argv() -> list[str]:
    """Resolve the ``little`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$LITTLE_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("little")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\little.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m little_cli.main`` — fallback for setups where
       Little is launched from a venv and the ``little`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_little_bin`` for the same reason. Kept
    local (not imported from gateway) because ``little_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("LITTLE_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _little_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _little_path_argv(resolved_env_bin)
        return _module_little_argv()

    little_bin = _safe_which_no_cwd("little") if _IS_WINDOWS else shutil.which("little")
    if little_bin:
        return _little_path_argv(little_bin)
    return _module_little_argv()


def _kanban_worker_skill_available(little_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``little -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset LITTLE_HOME means the worker falls back to the default root
    # home (``~/.little``), which ships the bundled skill.
    base = _Path(little_home) if little_home else (_Path.home() / ".little")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``little -p <profile> chat -q ...`` subprocess.
    ...
    ``LITTLE_KANBAN_DB`` / ``LITTLE_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject LITTLE_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `little -p <name>` the
    # _apply_profile_override() runs *before* little_constants is imported.
    # If LITTLE_HOME is absent from the child's env, get_little_home() falls
    # back to Path.home() / ".little" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from little_cli.profiles import resolve_profile_env
    try:
        env["LITTLE_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via LITTLE_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # LITTLE_HOME never had profiles created.
        pass
    if task.tenant:
        env["LITTLE_TENANT"] = task.tenant
    env["LITTLE_KANBAN_TASK"] = task.id
    env["LITTLE_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["LITTLE_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["LITTLE_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["LITTLE_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`little -p <name>`
    # rewrites LITTLE_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_little_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["LITTLE_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["LITTLE_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["LITTLE_KANBAN_BOARD"] = resolved_board
    # LITTLE_PROFILE is the author the kanban_comment tool defaults to.
    # `little -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["LITTLE_PROFILE"] = profile_arg

    cmd = [
        *_resolve_little_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped LITTLE_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("LITTLE_HOME")):
    ...
    # `little kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`little` executable not found on PATH. "
            "Install Little Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``little kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled LITTLE_PROFILE values like "little-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Little root exists

    Reads profile paths directly so this module has no import dependency on
    ``little_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from little_constants import get_default_little_root
        default_root = get_default_little_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``little kanban assignees`` for the terminal.
